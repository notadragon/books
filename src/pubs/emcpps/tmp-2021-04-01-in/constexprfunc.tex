...
%% JOSH: code listings that are inside a list item have less horizontal space because the entire item AND the code is indented even beyond that. Some lines of these code snippets are too long. Up to you whether to fix now or leave to fix during copyedit review. 
...
\begin{emcppslisting}
inline const int z() { return 5; }  // returns a nonconstant expression
...
\begin{emcppslisting}
constexpr int test(StoreForRt x) { return 0; }  // C++11: OK, is a literal type
constexpr void test(StoreForRt x) { }           // C++14: "     "  "    "      "
\end{emcppslisting}
...
static_assert(f(1),  "");  // Error, (ù{\codeincomments{i}}ù) is *not* usable in a constant expression.

static_assert(g(mb), "");  // Error, (ù{\codeincomments{mb}}ù) is not usable in a constant expression.
static_assert(g(0),  "");  // OK
static_assert(g(1),  "");  // OK, (ù{\codeincomments{j}}ù) *is* usable in a constant expression.
...
\begin{emcppslisting}
constexpr int f4();
constexpr int f4() {/*...*/}  // OK, (ù{\codeincomments{constexpr}}ù) matching exactly

constexpr int f5() {/*...*/}
          int f5() {/*...*/}  // Error, (ù{\codeincomments{constexpr}}ù) missing

          int f6()
constexpr int f6() {/*...*/}  // Error, (ù{\codeincomments{constexpr}}ù) added
\end{emcppslisting}
...
provided* that it never actually gets evaluated at compile time:
...
\begin{emcppslisting}
constexpr int f(int) { /*...*/ }  // OK
int f(int)           { /*...*/ }  // Error, (ù{\codeincomments{int f(int)}}ù) is now multiply defined.
...
public:
    constexpr Point1(int x, int y) : d_x(x), d_y(y) { }  // OK, *is* (ù{\codeincomments{constexpr}}ù)

    constexpr int x()       { return d_x; }  // OK, *is* (ù{\codeincomments{constexpr}}ù)
              int y() const { return d_y; }  // OK, is *not* (ù{\codeincomments{constexpr}}ù)
};
\end{emcppslisting}
...
constexpr int g(int x)
{
    ...
    using S = struct { };     // Error, no aliases that defined types
...
struct D3 : C  // public derivation
{
    ...
    constexpr D3() { }  // Error, (ù{\codeincomments{d\_i}}ù) initialized using non(ù{\codeincomments{constexpr}}ù) function

    constexpr D3(int i) : d_i(i) { }  // OK, (ù{\codeincomments{d\_i}}ù) set from initializer list
};
\end{emcppslisting}
...
struct D : virtual B  // Virtual base classes preclude (ù{\codeincomments{constexpr}}ù) constructors.
{
...
struct S2
{
    ...
    S2(char c) : d_s1(c) { }           // value constructor
       // OK, this constructor is *not* declared to be (ù{\codeincomments{constexpr}}ù).
};
\end{emcppslisting}
...
\begin{emcppslisting}
struct I0  { int i; /* implicit default ctor */ };  // OK, (ù{\codeincomments{I0}}ù) is a literal type.

struct I1a { int i;            I1a()        { } };  // OK, (ù{\codeincomments{i}}ù) is not initialized.
struct I1b { int i;  constexpr I1b()        { } };  // Error, (ù{\codeincomments{i}}ù) not initialized.

struct I2a { int i;            I2a() = default; };  // OK, but not (ù{\codeincomments{constexpr}}ù)
struct I2b { int i;  constexpr I2b() = default; };  // Error, (ù{\codeincomments{i}}ù) is not initialized.

struct I3a { int i;            I3a() : i(0) { } };  // OK, (ù{\codeincomments{i}}ù) is initialized.
struct I3b { int i;  constexpr I3b() : i(0) { } };  // OK, (ù{\codeincomments{I3b}}ù)'s a literal type.

struct S0  { I3b i; /* implicit default ctor */ };  // OK, (ù{\codeincomments{S0}}ù) is a literal type.

struct S1a { I3b i;            S1a()        { } };  // OK, (ù{\codeincomments{i}}ù) is initialized.
struct S1b { I3b i;  constexpr S1b()        { } };  // OK, (ù{\codeincomments{S1b}}ù)'s a literal type.

struct S2a { I3b i;            S2a() = default; };  // OK, (ù{\codeincomments{S2a}}ù)'s a literal type.
struct S2b { I3b i;  constexpr S2b() = default; };  // OK, (ù{\codeincomments{S2b}}ù)'s a literal type.
\end{emcppslisting}
    

\noindent The example code above attempts to illustrate the subtle differences
between a data member of \emph{scalar }literal type** (e.g.,
\lstinline!int!) and one of \emph{user-defined }literal type** (e.g.,
...
struct S
{
    ...
    constexpr S(const V& x, double y) : i(x), d(y) { }  // Error, the needed
        // (ù{\codeincomments{int}}ù) implicit conversion is *not* declared (ù{\codeincomments{constexpr}}ù).

    constexpr S(int x, const V& y) : i(x), d(y) { }    // OK, the needed
        // (ù{\codeincomments{double}}ù) implicit conversion *is* declared (ù{\codeincomments{constexpr}}ù).
};
\end{emcppslisting}
}
\end{enumerate}
...
struct S0 { int i;           S0() : i(0) { } };  // (ù{\codeincomments{S0}}ù) is *not* a *literal type*.
struct S1 { int i; constexpr S1() : i(0) { } };  // (ù{\codeincomments{S1}}ù) *is* a *literal type*.

int a[sizeOf(int())];  // OK,    (ù{\codeincomments{int}}ù) *is*     a *literal type*.
int b[sizeOf( S0())];  // Error,  (ù{\codeincomments{S0}}ù) is *not* a *literal type*.
int c[sizeOf( S1())];  // OK,     (ù{\codeincomments{S1}}ù) *is*     a *literal type*.
\end{emcppslisting}
...
\begin{emcppslisting}
constexpr void f(const volatile void) { };  // legal syntax as of C++14
\end{emcppslisting}
} 
\end{enumerate}   
...
static_assert(processByValue(Lt(7)) == 0, "");  // OK (because of elided copy)
...
\noindent In the code example above, we have an \emph{aggregate }literal type**,
...
constructing an object of \lstinline!Lt! from a literal \lstinline!int! value
enables the compiler to elide the copy, and all is well. When we
consider using an independently constructed \lstinline!constexpr! variable,
...
compile-time assertion fails to compile; see \featureref{\locationc}{constexprvar}.}
...
non\emph{movable }literal types**:
...
written as
  \lstinline!constexpr!~\lstinline!void!~\lstinline!test(S)!~\lstinline!{!~\lstinline!}!,
...
public:
    constexpr LiteralType(int i) : d_value(i != 75033 ? throw 0 : i) { }  // OK
};

constexpr int checkLiteral(LiteralType) { return 0; }  // OK, *literal type*
\end{emcppslisting}
...
\begin{emcppslisting}
char a[(LiteralType(75033), 1)];               // OK, usable in constant expr
static_assert((LiteralType(75033), true), ""); // OK,   "    "     "       "
\end{emcppslisting}
...
constexpr std::size_t constStrlenLit(char (&lit)[N])
{
    return N - 1;
}
