...
\emcppsgloss[user defined type]{user-defined type (UDT)} --- i.e., \lstinline!class!, \lstinline!struct!, or
\lstinline!union! --- that invokes another constructor defined for the same
\emcppsgloss{UDT} as part of its initialization of an object of that type.
...
the object being initialized is considered only \emcppsgloss{partially constructed} (i.e., the object is not yet known to be in a valid state),
...
\noindent Although the destructor of a \emcppsgloss{partially constructed} object will not be invoked, the destructors of
...
constructor, the object being initialized is considered \emcppsgloss{fully constructed}, as the target constructor must have returned control to
...
make use of \emcppsgloss{member initialization lists} to initialize base classes and data members efficiently. As of C++11, \emph{delegating
...
directly through the \emcppsgloss{member initialization list}.
...
program is \emcppsgloss[ill formed, no diagnostic required]{ill formed, no diagnostic required (IFNDR)}. While some compilers can, under certain conditions, detect delegation cycles at compile time, they are neither
