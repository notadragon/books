...
\romeogloss{integral type} used to represent its enumerated values, which
can be specified explicitly in C++11.

\subsection[Description]{Description}\label{description}

Every enumeration employs an integral type, known as its
\romeogloss{underlying type}, to represent its compile-time-enumerated
values. By default, the \romeogloss{underlying type} of a C++98
...
As of C++11, we have the ability to specify the \romeogloss{integral type}
...
outside the boundaries of what the provided \romeogloss{underlying type} is
...
enumerators convert automatically to their integral \romeogloss{underlying
...
12 bytes on some relevant platforms due to \romeogloss{natural alignment}!
...
an 8-bit signed integer, we can specify its \romeogloss{underlying type} to
...
it with its \romeogloss{underlying type} but without its enumerator list. If
...
definition and the \emph{new} full one would become silently ill formed, no diagnostic required (\romeogloss{IFNDR}). (See \featureref{\locationc}{enumopaque}.)
...
\romeogloss{underlying type}, which is not always the case. When used in a
...
\romeogloss{integral promotion} comes into play. For unscoped enumerations
...
\noindent Casting to the \romeogloss{underlying type} is \emph{not} necessarily the
...
\item{\seealsoref{enumclass}{\seealsolocationc}Introduces a scoped, more strongly typed enumeration that can explicitly specify an underlying type.}
