...
\romeogloss{local scope}).  A variable not marked \lstinline!static! is known as
an \romeogloss{automatic} variable, with a distinct instance on the
\romeogloss{program stack} for each function invocation.  \romeogloss{Automatic} objects
are allocated on the stack and initialized whenever the \romeogloss{flow of control}
...
if the function was called concurrently from multiple threads.   These \romegloss{data races}
...
operating system. After that, a second thread might begin initializing \lstline!str! again --- because
