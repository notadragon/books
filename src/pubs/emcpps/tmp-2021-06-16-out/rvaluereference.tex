...
% 21 June 2021, JMB code check complete
...
some of the common properties amongst the value categories: \textbf{[AUs: do you want to continue the previous numbering and make these 4 and 5?  notadragon: My preference would be to make these itemize not enumerate and use bulleted lists, there is no a priori ordering on these items.  If forced to use numbers, then yes, the numbers should be contiguous (1-3, 4-5)]}
\begin{enumerate}
...
\begin{emcppshiddenlisting}[emcppsbatch={e5,e10,e11}]
#include <utility>  // (ù{\codeincomments{std::move}}ù)
...
%%%
[\textbf{AUs: there is no subsection called ``lifetime extension"; did you mean ``Lifetime extension of a temporary bound to a reference"?  notadragon: Yes, lifetime-extension-of-a-temporary-bound-to-a-reference}]
%%%
...
void testExpressions()
{
    ...
    static_cast<int>(x);    // (ù{\codeincomments{int(x)}}ù) is a (ù{\emphincomments{prvalue}}ù) of type (ù{\codeincomments{int}}ù).
}
\end{emcppslisting}
...
\begin{emcppslisting}[emcppsbatch=e6]
int&& rf();  // (ù{\codeincomments{rf()}}ù) is an (ù{\emphincomments{xvalue}}ù) of type (ù{\codeincomments{int}}ù).
S&& rg();    // (ù{\codeincomments{rg()}}ù) is an (ù{\emphincomments{xvalue}}ù) of type (ù{\codeincomments{S}}ù).
...
void test5()
{
    ...
    assert(&ru != static_cast<void*>(&u));  // (ù{\codeincomments{ru}}ù) refers to a different (ù{\codeincomments{char}}ù).
...
\caption{Parameter Types}\label{rvalueref-table1} \vspace{1.5ex}
...
\begin{center}
\begin{threeparttable}
\caption{Overload Resolution Priorities}\label{rvalueref-table2} \vspace{1.5ex}
...
is provided here for concreteness; see \featureref{\locationc}{forwardingref}, \featureref{\locationc}{constexprfunc}, 
and
...
initialized by the returned \romeovalue{prvalue}.
...
\begin{emcppslisting}[style=footcode,emcppsbatch=e10]
S returnRefParam(S&& in)
{
...
optimization levels and properties of the type in question.
...
% 
\textbf{[AUs: there is no section within Pitfalls called ``Pessimizing moves"; what did you intend? notadragon: returning-const-rvalues-pessimizes-performance]}
%
...
{
    S a;
    return 1, std::move(a);  // (ù{\codeincomments{std::move}}ù) needed
}
\end{emcppslisting}
...
\begin{emcppslisting}[emcppsbatch=e12]
struct T {
    T(S&&);  // (ù{\codeincomments{S}}ù) is convertible to (ù{\codeincomments{T}}ù).
};
...
move operations}). 
%%%
\textbf{[AUs: there is no section called ``UDTs having implicitly generated move operations"; what did you intend? notadragon: special-member-function-generation]}
%%%
...
\emph{move} operations"; what did you intend?  notadragon: special-member-function-generation]}
%%%
...
\begin{emcppslisting}[emcppsbatch=e14,emcppsoutputfile={my_string.cpp}]
...
\lstinline!Person2!, to be constructed in this way. We will, at a minimum,
need to provide a \emcppsgloss{value constructor} for that purpose:

\begin{emcppslisting}[emcppsbatch=e16]
struct Person2  // We want to add a value constructor.
{
    String firstName;
    String lastName;

    Person2(const char* first, const char* last)  // value constructor
...
\begin{emcppslisting}[emcppsbatch=e16]
Person2 w1("John", "Lakos");           // OK, invokes value constructor
Person2 x1;                            // Error, no default constructor
Person2 y1 = { "Vittorio" };           // Error, no longer an aggregate
Person2 z1 = { "Vittorio", "Romeo" };  // OK, calls value ctor as of C++11
\end{emcppslisting}
...
Since \lstinline!Person2! is no longer considered an aggregate, we cannot
...
\begin{emcppslisting}[emcppsbatch=e16]
struct Person2a  // We want to add a value constructor (Revision a).
{
    String firstName;
    String lastName;

    Person2a(const char* first, const char* last)  // value constructor
    : firstName(first), lastName(last) { }

    // New here in (ù{\codeincomments{Person2a}}ù).
    Person2a() = default;                          // default constructor
};
\end{emcppslisting}
    
    
\noindent Notice that \lstinline!Person2a! differs from \lstinline!Person2! in the
...
\begin{emcppslisting}[emcppsbatch=e16]
Person2a x1a("John", "Lakos");           // OK, invokes value constructor
Person2a y1a;                            // OK, invokes default constructor
Person2a z1a = { "Vittorio" };           // Error, still not an aggregate
Person2a w1a = { "Vittorio", "Romeo" };  // OK, calls value ctor as of C++11
\end{emcppslisting}
...
modified class \lstinline!Person3!:

\begin{emcppslisting}[emcppsbatch=e16]
struct Person3  // We want to employ a user-provided default constructor.
{
    String firstName;
    String lastName;

    Person3() { /* user-provided */ }  // default constructor
};
\end{emcppslisting}
...
\begin{emcppslisting}[emcppsbatch=e16]
Person3 w2("abc", "def");       // Error, no value constructor is provided.
Person3 x2;                     // OK, invokes user-provided default ctor
Person3 y2 = { "abc" };         // Error, not an aggregate and no 1-arg ctor
Person3 z2 = { "abc", "def" };  // Error, not an aggregate and no value ctor
\end{emcppslisting}
}
...
call the new class \lstinline!Person4!:

\begin{emcppslisting}[emcppsbatch=e16]
struct Person4  // We want to employ a user-provided destructor.
{
    String firstName;
    String lastName;

    ~Person4() { /* user-provided */ }  // destructor
};
\end{emcppslisting}
...
\begin{emcppslisting}[emcppsbatch=e16]
Person4 x3 = { "abc", "def" };  // OK, still an aggregate
Person4 y3;                     // OK, default constructor is still available.
Person4 z3(std::move(x3));      // Bug, invokes implicit copy constructor
void test4()
{
...
\emcppsgloss[default constructed]{default construct} (\lstinline!y3!) a \lstinline!Person4! object, no
...
\lstinline!Person4!, the corresponding implicitly generated \emph{copy}
...
%%%%%% NOTE: have AUs check above line and make sure the brackets are correct  notadragon: they were not, they are now
...
operations explicitly} in \lstinline!Person4a!:

\begin{emcppslisting}[emcppsbatch=e16]
struct Person4a  // Adding user-provided destructor (Revision a)
{
    String firstName;
    String lastName;

    ~Person4a() { /* user-provided */ }         // destructor

    Person4a(Person4a&&) = default;             // move constructor
    Person4a& operator=(Person4a&&) = default;  // move assignment
};
\end{emcppslisting}
...
\begin{emcppslisting}[emcppsbatch=e16]
Person4a x3a = {"abc", "def" };  // OK, still an aggregate
Person4a y3a;                    // Error, default constructor is not declared.
Person4a z3a(x3a);               // Error, copy constructor is deleted.
void test6()
{
...
e.g., \lstinline!Person4b!, restores the type to its previous functionality
as a proper aggregate:

\begin{emcppslisting}[emcppsbatch=e16]
struct Person4b  // Adding a user-provided destructor (Revision b)
{
    String firstName;
    String lastName;

    ~Person4b() { /* user-provided */ }              // destructor

    // Already added to (ù{\codeincomments{Person4a}}ù).
    Person4b(Person4b&&) = default;                  // move constructor
    Person4b& operator=(Person4b&&) = default;       // move assignment

    // New here in (ù{\codeincomments{Person4b}}ù).
    Person4b() = default;                            // default constructor
    Person4b(const Person4b&) = default;             // copy constructor
    Person4b& operator=(const Person4b&) = default;  // copy assignment
};
\end{emcppslisting}
    
    
\noindent To recap, adding the \emcppsgloss[user provided]{user-provided} \emcppsgloss{destructor} in
\lstinline!Person4! suppressed both \emcppsgloss{move operations}. Defaulting
these \emcppsgloss{move operations} in \lstinline!Person4a!, in turn, suppressed
...
Defaulting those three operations in \lstinline!Person4b! restores
...
%%%% item 5
\item{\emcppsgloss[user provided]{User-provided} \emcppsgloss{copy constructor}. Once we understand the consequences of defining our own \emcppsgloss{destructor} (e.g., \lstinline!Person!~\lstinline!3! in the most recent example), the story behind adding a \emcppsgloss{copy constructor} to our original aggregate \lstinline!Person! class will seem remarkably similar. Rather than repeat that laborious discovery process in all its detail, starting with \lstinline!Person5! (the original aggregate + the copy constructor, not shown), we’ll instead provide only the end result, namely \lstinline!Person5c! with all special member functionality restored:

\begin{emcppslisting}[emcppsbatch=e16]
struct Person5c  // adding a user-provided copy constructor
{
    String firstName;
    String lastName;

    Person5c(const Person5c& original)               // copy constructor
    : firstName(original.firstName)
    , lastName(original.lastName) { /* user-provided */ }

    // Already added to (ù{\codeincomments{Person5a}}ù) (not shown).
    Person5c() = default;                            // default constructor

    // Already added to (ù{\codeincomments{Person5b}}ù) (not shown).
    Person5c(Person5c&&) = default;                  // move constructor
    Person5c& operator=(Person5c&&) = default;       // move assignment

    // New here in (ù{\codeincomments{Person5c}}ù).
    Person5c& operator=(const Person5c&) = default;  // copy assignment
};
\end{emcppslisting}
...
\lstinline!Person5a!, by user-providing a \emcppsgloss{copy constructor} or a
...
\lstinline!Person5a!, we regain the ability to compile our unit test driver
...
\lstinline!Person5b! restores their respective move capabilities.
...
\lstinline!Person5c! (shown in the example above), we regain all of the
...
\lstinline!Person5c! no longer amenable to \emcppsgloss{aggregate
...
%%%% item 6
\item{\emcppsgloss[user provided]{User-provided} \emcppsgloss[copy assignment operator]{copy-assignment operator}. Once we understand the consequences of \emcppsgloss[user provided]{user-providing} a \emcppsgloss{copy constructor} (e.g., \lstinline!Person5c! in the example in item 4), there are no surprises here. Again, we’ll provide, for reference, only the final, transitive result, \lstinline!Person6b!:

\begin{emcppslisting}[emcppsbatch=e16]
struct Person6b  // adding a user-provided copy-assignment operator
{
    String firstName;
    String lastName;

    Person6b& operator=(const Person6b& rhs)    // copy assignment
    {
    ...
    Person6b(Person6b&&) = default;             // move constructor
...
\noindent Again, we've omitted \lstinline!Person6! and \lstinline!Person6a! from the
example above, but we'll walk through those revisions. Providing a
user-defined \emcppsgloss[copy assignment operator]{copy-assignment operator} in \lstinline!Person6!,
...
Restoring \emcppsgloss{move assignment} in \lstinline!Person6a! has no further
...
construction}. Class \lstinline!Person6b!, shown in the code snippet above,
...
struct Person7b  // adding a user-provided move constructor
{
    ...
    // Already added to (ù{\codeincomments{Person7a}}ù) (not shown).
...
In the original enhanced version (\lstinline!Person7a!, which is not shown
...
operator} in \lstinline!Person7a!.
...
class, namely \lstinline!Person7b! (shown in the code snippet above), but
...
\begin{emcppslisting}[emcppsbatch=e16]
struct Person8b  // adding a user-provided move-assignment operator
{
    String firstName;
    String lastName;

    Person8b& operator=(Person8b&& expiring)         // move assignment
    {
        firstName = std::move(expiring.firstName);
    ...
    // Previously added to (ù{\codeincomments{Person8a}}ù) (not shown).
    Person8b(const Person8b&) = default;             // copy constructor
    Person8b& operator=(const Person8b&) = default;  // copy assignment
    Person8b(Person8b&&) = default;                  // move constructor

    // New here in (ù{\codeincomments{Person8b}}ù).
    Person8b() = default;                            // default constructor
};
\end{emcppslisting}
    
    
\noindent Again, we've omitted \lstinline!Person8! and \lstinline!Person8a!, but we'll
...
version (\lstinline!Person8!), the \emcppsgloss[user provided]{user-provided}
...
functions} (\lstinline!Person8a!), we would discover that the defaulting of
...
(\lstinline!Person8b!), we regain all of the original functionality of the
...
\begin{emcppshiddenlisting}[emcppsbatch={e21,e22,e23,e24}]
#include <string>   // (ù{\codeincomments{std::string}}ù)
\end{emcppshiddenlisting}
...
public:
    ...
    // (ù{\codeincomments{url}}ù) is a sink argument
...
public:
    HttpRequest(const std::string& url) : d_url(url) { }  // as before

    HttpRequest(std::string&& url) : d_url{std::move(url)} { }

    void set_url(const std::string& url)  // as before
    {
        d_url = url;
    }
...
the constructor and the \lstinline!set_url! function. Having the extra
...
\lstinline!set_url! are essentially repeated. In addition, this approach
...
\lstinline!set_url!, the \romeovalue{lvalue} will be copied in the
\lstinline!url! argument, and then the argument will be moved into the
data member: There will be one copy plus one move. If a user passes an
\romeovalue{rvalue} to \lstinline!set_url!, the \romeovalue{rvalue} will be moved
into the the \lstinline!url! argument, and then the argument will be moved
...
\begin{emcppshiddenlisting}[emcppsbatch={e25,e26}]
#include <string>   // (ù{\codeincomments{std::string}}ù)
#include <cassert>  // standard C (ù{\codeincomments{assert}}ù) macro
\end{emcppshiddenlisting}
...
\begin{emcppshiddenlisting}[emcppsbatch=e33]
#include <utility>  // (ù{\codeincomments{std::move}}ù)
\end{emcppshiddenlisting}
...
\begin{emcppshiddenlisting}[emcppsbatch=e34]
#include <utility>  // (ù{\codeincomments{std::move}}ù)
class C { /*...*/ };
void processC(const C&);  // (ù{\emphincomments{lvalue}}ù) reference overload for processing (ù{\codeincomments{C}}ù) objects
void processC(C&&);       // (ù{\emphincomments{rvalue}}ù) reference overload for processing (ù{\codeincomments{C}}ù) objects
...
\begin{emcppshiddenlisting}[emcppsbatch=e35]
// --- Replace
...
public:
    S2();
    S2(const S2& original);
    S2& operator=(const S2& rhs);
    void set(int i);
    int get() const;
    S2(S2&& expiring);
    S2& operator=(S2&& expiring);
// --- End
\end{emcppshiddenlisting}
\begin{emcppslisting}[emcppsbatch=e35]
class S2
...
\begin{emcppshiddenlisting}[emcppsbatch=e36]
#include <utility>  // (ù{\codeincomments{std::swap}}ù)
// --- Replace
...
\begin{emcppshiddenlisting}[emcppsbatch=e37]
// --- Replace
    // ...          (identical to (ù{\codeincomments{S2}}ù) above)
    int* d_r_p;
...
\begin{emcppshiddenlisting}[emcppsbatch=e38]
// --- Replace
...
public:
    S5a();
    S5a(const S5a& original);
    S5a& operator=(const S5a& rhs);
    S5a(S5a&& expiring);
    S5a& operator=(S5a&& expiring);
// --- End
\end{emcppshiddenlisting}
\begin{emcppslisting}[emcppsbatch=e38]
#include <cstdlib>  // (ù{\codeincomments{std::rand}}ù)
class S5a
{
    // ...          (identical to (ù{\codeincomments{S4}}ù) above)
...
been requested:
\begin{emcppshiddenlisting}[emcppsbatch=e39]
#include <utility>  // (ù{\codeincomments{std::move}}ù)
#include <cassert>  // standard C (ù{\codeincomments{assert}}ù) macro
// --- Replace
    // ...          (identical to (ù{\codeincomments{S4}}ù) above)
...
public:
    S5b();
    S5b(const S5b& original);
    S5b& operator=(const S5b& rhs);
    void set(int i);
    S5b(S5b&& expiring);
    S5b& operator=(S5b&& expiring);
// --- End
\end{emcppshiddenlisting}
...
\begin{emcppshiddenlisting}[emcppsbatch=e40]
// --- Replace
    // ...          (identical to (ù{\codeincomments{S4}}ù) above)
    int* d_r_p;

public:
    S4b(const S4b& original);
    S4b& operator=(const S4b& rhs);
    S4b(S4b&& expiring);
    S4b& operator=(S4b&& expiring);
// --- End
\end{emcppshiddenlisting}
...
\begin{emcppshiddenlisting}[emcppsbatch=e42]
#include <utility>  // (ù{\codeincomments{std::move}}ù)
\end{emcppshiddenlisting}
...
\begin{emcppshiddenlisting}[emcppsbatch=e43]
#include <memory>  // (ù{\codeincomments{std::unique\_ptr}}ù), (ù{\codeincomments{std::make\_unique}}ù)
...
\end{emcppshiddenlisting}
\begin{emcppslisting}[emcppsbatch=e43,emcppsstandards={c++14}]
class implementation;
...
\begin{emcppslisting}[emcppsbatch=e44]
class my_type
{
    //...
public:
    //...
    my_type(my_type&&) noexcept = default;             // move constructor
    my_type& operator=(my_type&&) noexcept = default;  // move assignment
    // ...
};
\end{emcppslisting}
...
\begin{emcppshiddenlisting}[emcppsbatch=e45]
struct my_type
{
    my_type(int i);
};
\end{emcppshiddenlisting}
\begin{emcppslisting}[emcppsbatch=e45]
#include <vector>  // (ù{\codeincomments{std::vector}}ù)
...
\begin{emcppshiddenlisting}[emcppsbatch=e46]
struct my_type
{
    int *d_remote;
...
\end{emcppshiddenlisting}
\begin{emcppslisting}[emcppsbatch=e46]
...
\begin{emcppshiddenlisting}[emcppsbatch=e47]
struct my_type
{
    int *d_remote;
...
\end{emcppshiddenlisting}
\begin{emcppslisting}[emcppsbatch=e47]
my_type& my_type::operator=(const my_type& rhs)
{
    if (d_remote == nullptr)
    {
...
\begin{emcppshiddenlisting}[emcppsbatch=e48]
#include <utility>
struct my_type
{
};
\end{emcppshiddenlisting}
\begin{emcppslisting}[emcppsbatch=e48]
void test2()
{
    my_type a;
...
\begin{emcppslisting}[emcppsbatch=e49]
...
\begin{emcppslisting}[emcppsbatch=e50]
...
Point *address = &singletonPoint();  // address of function-local (ù{\codeincomments{static Point}}ù).
\end{emcppslisting}
...
\begin{emcppslisting}[emcppsbatch=e51]
...
\begin{emcppshiddenlisting}[emcppsbatch=e52]
#include <iostream>
\end{emcppshiddenlisting}
\begin{emcppslisting}[emcppsbatch=e52]
...
\textbf{[AUs: was the ! in the title intended? notadragon: yes, it's somewhat surprising.  ]}
...
\begin{emcppshiddenlisting}[emcppsbatch=e53]
#include <cassert>  // standard C (ù{\codeincomments{assert}}ù) macro
\end{emcppshiddenlisting}
\begin{emcppslisting}[emcppsbatch=e53]
struct S
{
    int d_i;
...
is represented in the Standard\footnote{\textbf{[AUs: Where in the standard? cite.  notadragon: It is at the start of section 3.10 [basic.lval].  I believe it is page 78  ]}} with the following
...
like our \linebreak%%%%%
\lstinline!std::vector<vector<std::string>>! example
described in \intrarefsimple{rationale:-why-do-we-want-move-semantics}.
To enable such cases, \romeovalue{xvalues} also needed to
