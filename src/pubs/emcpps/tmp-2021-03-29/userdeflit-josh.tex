...
A \emcppsgloss{literal} is a single token in a program that represents, in C and classic C++, a
...
\emcppsgloss{factory function} and, unlike literals of built-in types, these
...
a highly constrained parameter list (see \intrarefsimple{user-defined-literal-(udl)-operators}) 
and returns an
...
\noindent We are not done yet. Unlike built-in literals, \emcppsgloss[user defined literal (UDL)]{UDLs} that use the suffixes defined above cannot express arbitrary literal values
...
The allowed variations on the definitions of \emcppsgloss[UDL operator]{UDL operators}
...
the three forms of \emcppsgloss[UDL operator]{UDL operators} (see \intrarefsimple{prepared-argument-udl-operators}, \intrarefsimple{raw-udl-operators}, and \intrarefsimple{templated-udl-operators}) needs, or
...
of the \emcppsgloss{naked literal} strings. If more than one of the strings has a
...
operators} is expounded in more detail in its own separate section; see \intrarefsimple{prepared-argument-udl-operators}, \intrarefsimple{raw-udl-operators}, and \intrarefsimple{templated-udl-operators}.
...
directive]{\lstinline!using! directive}. We could have avoided the nested namespace and, instead, required each needed operator to be imported individually:
...
category} of the \emcppsgloss{UDL operator} refers to only the \emcppsgloss{naked
...
be used on character literals; see  \featureref{\locationa}{unicode-string-and-character-literals}.{\cprotect\footnote{C++17 allows the \lstinline!u8! prefix on
...
int         c1 =  'Q'_ee;  // OK, matches (ù{\codeincomments{char}}ù) parameter type
double      c2 = U'Q'_ee;  // OK, matches (ù{\codeincomments{char32\_t}}ù) parameter type
const char* c3 = u'Q'_ee;  // Error, no match for (ù{\codeincomments{char16\_t}}ù) parameter type
Smile       c4 = L'Q'_ee;  // Error, no match for (ù{\codeincomments{wchar\_t}}ù) parameter type
\end{emcppslisting}
...
for a single \emcppsgloss{UDL suffix}, each having a distinct type category
...
\emcppsgloss[UDL operator]{UDL operators} that differ by character type (\lstinline!char!,
...
operator} for string literals by the \emph{absence} of an
...
\noindent Note that we could have declared our \lstinline!_3! \emcppsgloss{raw
UDL operator} \lstinline!constexpr! and replaced all of the (runtime)
\lstinline!assert! statements with (compile-time) \lstinline!static_assert! declarations. Also observe that, as written, \lstinline!constexpr!~\lstinline!operator""!~\lstinline!_3! requires C++14's relaxed \lstinline!constexpr! function restrictions (see \featureref{\locationc}{constexprfunc}), but it could be reimplemented to work in C++11.
...
would otherwise overflow or lose precision confers additional expressiveness on raw UDLs; e.g., the base-3 \emcppsgloss{raw UDL operator} shown above could
not be expressed using a \emcppsgloss{prepared-argument UDL operator}. The downside of this additional expressiveness is the need to implement, debug, and maintain custom token parsing, including error checking.

If the intent is to consume integer literals, a \emcppsgloss{raw UDL
operator} needs to  either process or reject not only the
...
%\intrarefnopage{templated-udl-operators}.
...
literals in \intraref{use-cases-userdeflit}{user-defined-numeric-types} provides a more complete exposition of this sort
...
As template arguments, the characters that make up the \emcppsgloss{naked
...
providing proper \emcppsgloss{copy semantics}, equality comparison, variable sizing,
    ...
    // Read instructions from input and process them.
{
    ...
    if (isSafeString(instructions))
    {
       // The (ù{\codeincomments{instructions}}ù) string is considered safe; sanitize it.
       SanitizedString sanInstr = SanitizedString::fromRawString(instructions);
       // ...
...
\noindent The \lstinline!BigNum! class is appropriate for large integers, but numbers having 
...
fixed-point class template was proposed for standardization in \mbox{\cite{mcfarlane19}}.}}:
...
public:
    ...
    constexpr FixedPoint(long long data, std::true_type /*isRaw*/)
        // Create a (ù{\codeincomments{FixedPoint}}ù) object with the specified (ù{\codeincomments{data}}ù).  Note that
        // this is a "raw" constructor and no precision adjustment is made to
        // the (ù{\codeincomments{data}}ù).
        : d_data(data) { }

    friend std::ostream& operator<<(std::ostream& stream, const FixedPoint& v)
        // Format the specified fixed-point number (ù{\codeincomments{v}}ù), write it to the
        // specified (ù{\codeincomments{stream}}ù), and return (ù{\codeincomments{stream}}ù).
    {
...
\noindent Our data representation is a \lstinline!long!~\lstinline!long! (see \featureref{\locationa}{long-long}), making the
...
\lstinline!FixedPoint<3>!. We must first define a variadic helper template (see \featureref{\locationc}{variadictemplate}) to
...
A more robust way to use \emcppsgloss[user defined literal (UDL)]{UDLs} to express units is to
...
\subsubsection[Verbose usage]{Verbose usage}
...
\noindent The authors of the \lstinline!_date! UDL should provide its same functionality as a function or constructor to ensure that users do not always require a possibly over-verbose \emcppsgloss[using directive]{\lstinline!using! directive} to achieve their goals:
...
\noindent If the intention is to define a prepared-argument UDL where numbers with and without a decimal point are accepted, then both forms of the \emcppsgloss{UDL operator} must be defined.
...
\begin{itemize}
\item{\seealsoref{decltype}{\seealsolocationa}introduces a keyword often helpful for deducing the return type of a \emcppsgloss{templated UDL operator}.}
\item{\seealsoref{null-pointer-literal-(nullptr)}{\seealsolocationa}describes a keyword that unambiguously denotes the null pointer literal.}
\item{\seealsoref{auto}{\seealsolocationc} shows how type inference can be used to declare a variable to hold the value of a \emcppsgloss[user defined literal (UDL)]{UDL} when the type of the \emcppsgloss[user defined literal (UDL)]{UDL} varies based on its contents.}
\item{\seealsoref{constexprfunc}{\seealsolocationc}describes how most \emcppsgloss[user defined literal (UDL)]{UDLs} can be used as part of a constant expression.}
\item{\seealsoref{variadictemplate}{\seealsolocationc}shows how templates can take an infinite number of parameters, which is required for implementing \emcppsgloss[templated UDL operator]{templated UDL operators}.}
\item{\seealsoref{inline-namespaces}{\seealsolocatione}describes a feature not recommended for \emcppsgloss[UDL operator]{UDL operators}, yet the C++14 Standard Library puts \emcppsgloss[UDL operator]{UDL operators} into \lstinline!inline! namespaces.}
\end{itemize}
