...
\noindent Variable templates, like \romeogloss{C-style functions}, may be declared at
...
parameters (including a \romeogloss{parameter pack}):
...
A \romeogloss{type trait} is an empty type carrying compile-time information
...
in this use case decreases the amount of \romeogloss{boilerplate code} ---
...
desired goal, does not consume memory in the \romeogloss{static data
...
\romeogloss{template template class parameter}, no equivalent
...
accepting a \romeogloss{template template parameter}:
...
\seealsoref{constexprvar}{\seealsolocationc}Conditionally safe C++11 feature providing an alternative to \lstinline!const! template variables that can reduce unnecessary consumption of the \romeogloss{static data space}.}
\end{itemize}
