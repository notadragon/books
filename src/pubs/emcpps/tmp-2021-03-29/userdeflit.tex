...
\begin{emcppslisting}
constexpr std::time operator""_Min(unsigned long long m)  // (ù{\codeincomments{constexpr}}ù) function
{
    return static_cast<std::time_t>(m * 60);  // minutes-to-seconds conversion
}

constexpr std::time operator""_Hr(unsigned long long h)  // (ù{\codeincomments{constexpr}}ù) function
{
    return static_cast<std::time_t>(h * 3600);  // hours-to-seconds conversion
}
...
namespace ns1  // namespace containing types returned by UDL operators
{
    ...
    using namespace literals;  // Make literals available in (outer) (ù{\codeincomments{ns1}}ù) namespace
}
...
provides additional features related to built-in literals; see \featureref{\locationb}{binary-literals} and \featureref{\locationb}{digitseparator}:
...
\emcppsgloss[encoding prefix]{encoding prefixes}: \lstinline!u! to indicate \lstinline!char16_t!
...
\begin{emcppslisting}
#include <stdexcept>  // (ù{\codeincomments{std::out\_of\_range}}ù) and (ù{\codeincomments{std::overflow\_error}}ù)
...
std::u8string  s3 = u8"hello"s;   // value (ù{\codeincomments{std::u8string(u8"hello")}}ù)
...
complex<double>      c1 = 2.4 + 3i;    // value (ù{\codeincomments{complex<double>(2.4, 3.0)}}ù)
complex<long double> c2 = 1.2 + 5.1l;  // value (ù{\codeincomments{complex<long double>(1.2L, 5.1L)}}ù)
complex<float>       c3 = 0.1 + 2.if;  // value (ù{\codeincomments{complex<float>(0.1F, 2.0F)}}ù)
\end{emcppslisting}
...
\begin{emcppslisting}
#include <cmath>  // (ù{\codeincomments{std::sin}}ù) and (ù{\codeincomments{std::cos}}ù)
