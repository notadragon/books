...
std::pair<int, std::pair<double, std::string>> v;
    // Define a holder of an (ù{\codeincomments{int}}ù), a (ù{\codeincomments{double}}ù), and an (ù{\codeincomments{std::string}}ù), accessed as
...
\intraref{use-cases-variadic}{} \textbf{ [AUs: There is no subsection called ``Factory functions." Did you mean the Object factories section?]} ).
...
\intrarefsimple{} \textbf{[AUs: There is no section called ``Non-type parameter packs"]}), and \emcppsgloss[template template parameter pack]{template
template parameter packs} (see \intrarefsimple{} \textbf{[AUs: There is no section called ``Template template
parameter packs"]}). The phrase \emcppsgloss{parameter pack} is a conflation
...
template <typename... Ts> class D
{
    void memberFunction()
    {
        ...
        D<C<Ts...>> v1;       // OK, expand to (ù{\codeincomments{C1<C<T0, T1>, ...>}}ù)
        D<C<Ts>...>  v2;      // OK, expand to (ù{\codeincomments{C1<C<T0>, C1<T1>, ...>}}ù)
        D<vector<Ts>...> v3;  // OK, expand to (ù{\codeincomments{C1<std::vector<T0>, ...>}}ù)
    }
};
\end{emcppslisting}
...
\begin{emcppslisting}
template <class Key, class Value> class Map;  // similar to (ù{\codeincomments{std::map}}ù)
\end{emcppslisting}
...
int s1d = fs1(S1<int>(), S<bool>());  // (ù{\codeincomments{Ts}}ù)=<(ù{\codeincomments{int,bool}}ù)>
\end{emcppslisting}
...
template <class T> void print(const T& value);  // prints (ù{\codeincomments{x}}ù) to (ù{\codeincomments{stdout}}ù)
...
struct C3
    ...
    , X<double>...  // inherit (ù{\codeincomments{X0<double>}}ù), (ù{\codeincomments{X1<double>}}ù), ...
{ };
\end{emcppslisting}
...
C4<A5> err1;  // matches, but instantiations (ù{\codeincomments{A5<int>}}ù) and (ù{\codeincomments{A5<double>}}ù) will fail
C4<A6> err2;  // matches, but instantiations (ù{\codeincomments{A6<int>}}ù) and (ù{\codeincomments{A6<double>}}ù) will fail
\end{emcppslisting}
...
useful contexts are supported, however; see \intraref{Annoyances-variadic}{}\textbf{[ AUs: There is no section called ``Expansion at the statement level is not permitted." ]}
...
{
    size_t x1 = sizeof...(Ts);  // (ù{\codeincomments{x1}}ù) is the number of arguments of (ù{\codeincomments{f1}}ù).
    size_t x2 = sizeof...(xs);  // same value as (ù{\codeincomments{x1}}ù)
    int a1[sizeof...(Ts)];      // (ù{\codeincomments{sizeof...(Ts)}}ù) is a compile-time constant.
    int a2[sizeof...(xs)];      // (ù{\codeincomments{sizeof...(vs)}}ù) is a compile-time constant.
...
size_t nogo(Ts... xs)
{
    size_t x1 = sizeof 42;         // OK, same as (ù{\codeincomments{sizeof(int)}}ù)
    size_t x2 = sizeof... Ts;      // Error, parens required around (ù{\codeincomments{Ts}}ù)
    size_t x3 = sizeof... xs;      // Error, parens required around (ù{\codeincomments{xs}}ù)
    size_t x4 = sizeof(Ts...);     // Error, cannot expand inside (ù{\codeincomments{sizeof}}ù)
    size_t x5 = sizeof...(Ts...);  // Error, cannot expand inside (ù{\codeincomments{sizeof...}}ù)
    size_t x6 = sizeof(xs...);     // Error, cannot expand inside (ù{\codeincomments{sizeof}}ù)
    size_t x7 = sizeof...(xs...);  // Error, cannot expand inside (ù{\codeincomments{sizeof...}}ù)
}
\end{emcppslisting}
...
\noindent \intraref{annoyances-variadic}{}\textbf{[AUs: There is no subsection called ``Expansion at the statement level is not permitted"]}} discusses this context further. Also recall
...
\begin{emcppslisting}
template <size_t n, typename... Ts>  // (ù{\codeincomments{n}}ù) is the index. (ù{\codeincomments{Ts}}ù) is the tuple pack.
...
\begin{emcppslisting}
size_t i1 = IndexOf<int, int, long>::value;         // (ù{\codeincomments{i1}}ù) is 0
size_t i2 = IndexOf<int, short, int, long>::value;  // (ù{\codeincomments{i2}}ù) is 1
size_t i3 = IndexOf<const int, short, int>::value;  // (ù{\codeincomments{i3}}ù) is 1
...
Variant<Ts...>::Variant(T&& xs)             // uses perfect forwarding
{
    typedef std::remove_reference_t<T> U;
        // Remove reference from (ù{\codeincomments{T}}ù) if any, e.g. transform (ù{\codeincomments{int\&}}ù) into (ù{\codeincomments{int}}ù).
    ::new(&d_data) U(std::forward<T>(xs));  // Construct object at address.
    d_active = IndexOf<U, Ts...>::value;    // This code fails if (ù{\codeincomments{T}}ù) not in (ù{\codeincomments{Ts}}ù).
}
\end{emcppslisting}
...
\begin{emcppslisting}
template <class T, class... Ts>                  // (ù{\codeincomments{T}}ù) is the assumed type.
...
\begin{emcppslisting}
class FastDeleter     // policy for performing minimal checking on (ù{\codeincomments{SafePtr}}ù)
{ /* ... */ };

class CheckedDeleter        // policy for performing maximal checking on (ù{\codeincomments{SafePtr}}ù)
{ /* ... */ };
...
static_assert(std::is_same_v<
    ...
    Widget,                          // should be (ù{\codeincomments{Widget}}ù)
"");

static_assert(std::is_same_v<
    PointerTraits<WP1>::value_type,  // Fetch the pointee type of (ù{\codeincomments{WP2}}ù).
    Widget,                          // should also be (ù{\codeincomments{Widget}}ù)
"");
\end{emcppslisting}
...
static_assert(std::is_same_v<
    ...
    SafePtr<Gadget, CheckedDeleter>>,    // fully checked, just like (ù{\codeincomments{WP1}}ù)
"");

static_assert(std::is_same_v<
    PointerTraits<WP2>::rebind<Gadget>,  // Rebind (ù{\codeincomments{WP2}}ù) to (ù{\codeincomments{Gadget}}ù).
    SafePtr<Gadget, FastDeleter>>,       // minimally checked, like (ù{\codeincomments{WP2}}ù)
"");
\end{emcppslisting}
...
\featureref{}{} \textbf{[AUs: You have ``lambda expressions" here. Do you mean the lamba, lambda capture, or generic lambda feature? ]}{\cprotect\footnote{C++17 adds \emph{fold expressions} that allow easy \emcppsgloss{pack expansion}
