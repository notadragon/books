...
%@LORI: Note for typesetting: this code snippet is the one that should
%have final typeset as normal code and not a keyword. Exactly two of the
%uses of final here should end up bold.
%
%@LORI: THE `FINAL' IN UPPERCASE SHOULD BE CHANGED TO A LOWERCASE
%\texttt{final} THAT LOOKS LIKE LIKE ANY GENERIC CODE WORD, THE ONE IN
%LOWERCASE SHOULD LOOK LIKE A KEYWORD (SAME AS \texttt{override} or
%\texttt{virtual} or \texttt{const}):
...
%%%%% JOSH,  do you want to keep the quotes around "final" in the comments? 
\begin{emcppslisting}
...
void test()  // test driver that orchestrates mock implementation to test (ù{\codeincomments{File}}ù)
{
    ...
    FileHandle* fh = f.open("dummyPath");  // (ù{\emphincomments{mock}}ù) used to supply handle
    // ...
}
\end{emcppslisting}
...
%%%%% JOSH,  "final" here was set like it is in a comment, so it's blue; that doesn't seem right. If you want it to be black but not bold, just change it to (ù{\texttt{final}}ù)
\begin{emcppslisting}
client          Client  MockMechanism   Mechanism   ((ù{\codeincomments{final}}ù) virtual functions)
   o                 o        |          /
   |       =>         \       |         /
   |                   \      V        V
...
%%%%% JOSH,  "final" here was set like it is in a comment, so it's blue; that doesn't seem right. If you want it to be black but not bold, just change it to (ù{\texttt{final}}ù)
\begin{emcppslisting}
client          Client  MockMechanism  (making these (ù{\codeincomments{final}}ù) is pointless)
   o                 o       |
   |       =>         \      |  (BAD IDEA)
   |                   \     V
...
hierarchy}{\cprotect\footnote{\cite{lakos96}, Appendix~A, pp.~737--768}}{\cprotect\footnote{\cite{lakos2y}, section~2.7 [AUs: The citation is lakos2y, do you mean Vol II of Large-Scale C++?]}} is a generalization of the \emcppsgloss{composite
...
void client()
{
    ...
    TypeA(&a);  // (ù{\codeincomments{deallocate}}ù) is an empty (ù{\codeincomments{virtual}}ù) function call.
    TypeB(&a);  // (ù{\codeincomments{deallocate}}ù) is an empty (ù{\codeincomments{inline}}ù) function call.
}
\end{emcppslisting}
