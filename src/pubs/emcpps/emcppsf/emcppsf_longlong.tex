...
\lstinline!long!~\lstinline!long! is a \emcppsgloss{fundamental integral type} guaranteed to have at least 64 bits on all
platforms.

\subsection[Description]{Description}\label{description}

The \emcppsgloss{integral type} \lstinline!long!~\lstinline!long! and its companion
type \lstinline!unsigned!~\lstinline!long!~\lstinline!long! are the only two
\emcppsgloss{fundamental integral types} in C++ that are guaranteed to have
...
continue to evolve) over time, see \intraref{longlong-appendix}{historical-perspective-on-the-evolution-of-use-of-fundamental-integral-types}.
...
\emcppsgloss{signed integer overflow}, which is both \emcppsgloss{undefined
...
not yet generally considered a \emcppsgloss{vocabulary type} (see
\intraref{longlong-appendix}{historical-perspective-on-the-evolution-of-use-of-fundamental-integral-types}),
...
#include <cstdint>  // (ù{\codeincomments{std::int64\_t}}ù)
\end{emcppshiddenlisting}
...
\emcppsgloss{vocabulary types} becomes much more important (e.g., for
interoperability).
...
As discussed at some length in \intraref{longlong-appendix}{historical-perspective-on-the-evolution-of-use-of-fundamental-integral-types},
%{\it\titleref{longlong-appendix}} on page~\pageref{longlong-appendix},
the
