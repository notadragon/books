...
struct D0 : B0
{
    using B0::B0;  // (Ã¹{\codeincomments{using}}Ã¹) declaration
    D0(double d);  // suppress implicit default constructor
};

D0 t(1);     // OK, inherited from (Ã¹{\codeincomments{B0::B0(int)}}Ã¹)
D0 u(2, 3);  // OK, inherited from (Ã¹{\codeincomments{B0::B0(int, int)}}Ã¹)
D0 v("hi");  // Error: Base constructor is declared (Ã¹{\codeincomments{private}}Ã¹).
\end{emcppslisting}
...
\begin{emcppslisting}[language=C++]
#include <utility>  // (Ã¹{\codeincomments{std::move}}Ã¹)
...
D0 z(t);      // OK, uses compiler-generated (Ã¹{\codeincomments{D0::D0(const D0\&)}}Ã¹)
D0 j(D0(5));  // OK, uses compiler-generated (Ã¹{\codeincomments{D0::D0(D\&\&)}}Ã¹)
\end{emcppslisting}
...
void f()
{
    B0 b(0), bb(0);  // Create destination and source (Ã¹{\codeincomments{B0}}Ã¹) objects.
    D0 d(0), dd(0);  //   "         "       "    "    (Ã¹{\codeincomments{D0}}Ã¹)    ".

    b = bb;          // OK, assign base from lvalue base.
    b = B0(0);       // OK,   "       "   "  rvalue   "

    d = bb;          // Error: (Ã¹{\codeincomments{B0::operator=}}Ã¹) is hidden by (Ã¹{\codeincomments{D0::operator=}}Ã¹).
    d = B0(0);       // Error:       "         "    "     "        "
...
D1 d1(0);  // Error: Call of overloaded (Ã¹{\codeincomments{D1(int)}}Ã¹) is ambiguous.
\end{emcppslisting}
...
D3 d;        // prints (Ã¹{\codeincomments{"B3()"}}Ã¹)
D3 e(0);     // prints (Ã¹{\codeincomments{"D3(int)"}}Ã¹) --- The derived constructor is invoked.
D3 f(0, 0);  // prints (Ã¹{\codeincomments{"B3(int, int)"}}Ã¹)
\end{emcppslisting}
...
struct B1B { B1B(int); };  // provides a conversion constructor from an (Ã¹{\codeincomments{int}}Ã¹).

struct D3 : B1A, B1B
{
    using B1A::B1A;  // Inherit the (Ã¹{\codeincomments{int}}Ã¹) constructor from base class (Ã¹{\codeincomments{B1A}}Ã¹).
    using B1B::B1B;  // Inherit the (Ã¹{\codeincomments{int}}Ã¹) constructor from base class (Ã¹{\codeincomments{B1B}}Ã¹).

    D3(int i) : B1A(i), B1B(i) { }  // User-declare (Ã¹{\codeincomments{int}}Ã¹) conversion constructor
};                                  // that delegates to bases.

D3 d3(0);  // OK, calls (Ã¹{\codeincomments{D3(int)}}Ã¹)
\end{emcppslisting}
...
\begin{emcppslisting}[language=C++]
template <typename T>
struct S : T  // The base type, (Ã¹{\codeincomments{T}}Ã¹), is a *dependent type*.
{
...
{
    using std::vector<T>::vector;      // Inherit (Ã¹{\codeincomments{std::vector}}Ã¹)'s constructors.

    T& operator[](std::size_t index)   // Hide (Ã¹{\codeincomments{std::vector}}Ã¹)'s index operator.
    {
         assert(index < std::vector<T>::size());
         return std::vector<T>::operator[](index);
    }

    const T& operator[](std::size_t index) const  // Hide (Ã¹{\codeincomments{const}}Ã¹) index operator.
    {
         assert(index < std::vector<T>::size());
         return std::vector<T>::operator[](index);
    }
...
\begin{emcppslisting}[language=C++]
struct Birthday : Date  // somewhat type-safe alias for a (Ã¹{\codeincomments{Date}}Ã¹)
{
    using Date::Date;  // inherit (Ã¹{\codeincomments{Date}}Ã¹)'s three integer ctor
    explicit Birthday(Date d) : Date(d) { }  // (Ã¹{\codeincomments{explicit}}Ã¹) conversion from (Ã¹{\codeincomments{Date}}Ã¹)
};

struct Appointment : Date  // somewhat type-safe alias for a (Ã¹{\codeincomments{Date}}Ã¹)
{
    using Date::Date;  // inherit (Ã¹{\codeincomments{Date}}Ã¹)'s three integer ctor
    explicit Appointment(Date d) : Date(d) { }  // (Ã¹{\codeincomments{explicit}}Ã¹) conv. from (Ã¹{\codeincomments{Date}}Ã¹)
};
\end{emcppslisting}
...
Birthday b1 = d0;          // Error: no implicit conversion from (Ã¹{\codeincomments{Date}}Ã¹)
Appointment a0;            // Error: (Ã¹{\codeincomments{Appointment}}Ã¹) has no default ctor.
Appointment a1 = b0;       // Error: no implicit conversion from (Ã¹{\codeincomments{Birthday}}Ã¹)
Birthday n2(d0);           // OK, thanks to an (Ã¹{\codeincomments{explicit}}Ã¹) constructor in (Ã¹{\codeincomments{Birthday}}Ã¹)
Appointment a2(1999, 9, 17); // OK, thanks to inheriting constructors           
Birthday    b3(a2);          // OK, an (Ã¹{\codeincomments{Appointment}}Ã¹) (unfortunately) is a (Ã¹{\codeincomments{Date.}}Ã¹)
\end{emcppslisting}
...
{
    static int s_constructed;  // count of the number of (Ã¹{\codeincomments{T}}Ã¹) objects constructed
...
\begin{emcppslisting}[language=C++]
#include <string>   // (Ã¹{\codeincomments{std::string}}Ã¹)
#include <vector>   // (Ã¹{\codeincomments{std::vector}}Ã¹)
#include "myfoo.h"  // (Ã¹{\codeincomments{MyFoo}}Ã¹)

Counted<std::string>       cs ("ABC");  // Construct a counted (Ã¹{\codeincomments{string}Ã¹).          
Counted<std::vector<char>> cvc(3, 'a'); // Construct a counted vector of (Ã¹{\codeincomments{char}Ã¹).
Counted<MyFoo>             cmf;         // Construct a counted (Ã¹{\codeincomments{MyFoo}}Ã¹) object.
\end{emcppslisting}
...
public:
    PointOfInterest(const std::string& name, const std::string& address);
        // Please pass the (Ã¹{\codeincomments{name}}Ã¹) as the *first* and the (Ã¹{\codeincomments{address}}Ã¹) *second*!
};
\end{emcppslisting}
...
\begin{emcppslisting}[language=C++]
struct Name : std::string  // somewhat type-safe alias for a (Ã¹{\codeincomments{std::string}}Ã¹)
{
    using std::string::string;  // Inherit, as is, all of (Ã¹{\codeincomments{std::string}}Ã¹)'s ctors.
    explicit Name(const std::string& s) : std::string(s) { }  // conversion
};

struct Address : std::string  // somewhat type-safe alias for a (Ã¹{\codeincomments{std::string}}Ã¹)
{
    using std::string::string;  // Inherit, as is, all of (Ã¹{\codeincomments{std::string}}Ã¹)'s ctors.
...
Name n1 = s0;           // error: no implicit conversion from (Ã¹{\codeincomments{std::string}}Ã¹)
Address a0;             // OK, unfortunately a (Ã¹{\codeincomments{std::string}}Ã¹) has a default ctor.
Address a1 = n0;        // error: no implicit conversion from (Ã¹{\codeincomments{Name}}Ã¹)
Name n2(s0);            // OK, thanks to an (Ã¹{\codeincomments{explicit}}Ã¹) constructor in (Ã¹{\codeincomments{Name}}Ã¹)
Name b3(a0);            // OK, an (Ã¹{\codeincomments{Address}}Ã¹) (unfortunately) is a (Ã¹{\codeincomments{std::string}}Ã¹).
\end{emcppslisting}
...
struct NumericalDatum : Datum
{
    ...
    NumericalDatum(bool) = delete;  // ...except the one taking a (Ã¹{\codeincomments{bool}}Ã¹).
};
\end{emcppslisting}
...
{
private:
    Base(int) { }  // This constructor is declared (Ã¹{\codeincomments{private}}Ã¹) in the base class.
    void pvt0() { }
    void pvt1() { }

public:
    Base() { }     // This constructor is declared (Ã¹{\codeincomments{public}}Ã¹) in the base class.
    void pub0() { }
    void pub1() { }
};
\end{emcppslisting}
...
{
    using Base::Base;  // OK, inherited (Ã¹{\codeincomments{Base()}}Ã¹) as (Ã¹{\codeincomments{public}}Ã¹) constructor
                       // and (Ã¹{\codeincomments{Base(int)}}Ã¹) as (Ã¹{\codeincomments{private}}Ã¹) constructor

private:
    using Base::pub0;  // OK, (Ã¹{\codeincomments{pub0}}Ã¹) is declared (Ã¹{\codeincomments{private}}Ã¹) in derived class.
    using Base::pvt0;  // Error: (Ã¹{\codeincomments{pvt0}}Ã¹) was declared private in base class.

public:
    using Base::pub1;  // OK, (Ã¹{\codeincomments{pub1}}Ã¹) is declared (Ã¹{\codeincomments{public}}Ã¹) in derived class.
    using Base::pvt1;  // Error: (Ã¹{\codeincomments{pvt1}}Ã¹) was declared (Ã¹{\codeincomments{private}}Ã¹) in base class.
};

void client()
{
     Derived x(0);  // Error: Constructor was declared private in base class.
     Derived d;     // OK, constructor was declared (Ã¹{\codeincomments{public}}Ã¹) in base class.
     d.pub0();      // Error: (Ã¹{\codeincomments{pub0}}Ã¹) was declared (Ã¹{\codeincomments{private}}Ã¹) in derived class.
     d.pub1();      // OK, (Ã¹{\codeincomments{pub1}}Ã¹) was declared (Ã¹{\codeincomments{public}}Ã¹) in derived class.
     d.pvt0();      // Error: (Ã¹{\codeincomments{pvt0}}Ã¹) was declared private in base class.
     d.pvt1();      // Error: (Ã¹{\codeincomments{pvt1}}Ã¹) was declared private in base class.
}
\end{emcppslisting}
...
\begin{emcppslisting}[language=C++]
#include <utility>  // (Ã¹{\codeincomments{std::forward}}Ã¹)
...
%\begin{emcppslisting}[language=C++]
%S<int> s;   // full specialization of (Ã¹{\codeincomments{S}}Ã¹) for type (Ã¹{\codeincomments{int}}Ã¹)
...
%        // This constructor is enabled only if (Ã¹{\codeincomments{T}}Ã¹) is constructible from (Ã¹{\codeincomments{U}}Ã¹).
%    {
...
%{
%    S s;
%    Wrap<int> w(s);     // prints (Ã¹{\codeincomments{"S ctor"}}Ã¹)
%    Derived<int> d(s);  // error prior to fixes; prints (Ã¹{\codeincomments{"S ctor"}}Ã¹) afterward
%}
%\end{emcppslisting}
%    
...
%struct B          // base class
%{
%    B(int, int);  // value constructor with two (required) (Ã¹{\codeincomments{int}}Ã¹) parameters
%};
%
%struct D : B
%{
%    using B::B;
%    D(int, int, int = 0);  // doesn't suppress (Ã¹{\codeincomments{D(int, int)}}Ã¹) from (Ã¹{\codeincomments{B(int, int)}}Ã¹)
%};
%\end{emcppslisting}
...
%struct B : A
%{
%    using A::A;  // inherited base class (Ã¹{\codeincomments{A}}Ã¹)'s constructors
%};
%\end{emcppslisting}
...
%A x;
%B y = x;  // Surprise! This compiles, and it prints (Ã¹{\codeincomments{"copy"}}Ã¹) twice!
%\end{emcppslisting}
%    }
%
