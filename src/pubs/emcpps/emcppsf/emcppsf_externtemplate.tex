...
% 25 Feb 2021, updated gloss commands
% 18 March 2021, revisions in and proofed
...
Creating an \emcppsgloss[explicit instantiation
definition]{explicit-instantiation
definition} was possible prior to C++11.{\cprotect\footnote{The
  C++03 Standard term for the syntax used to create an
  \emcppsgloss[explicit instantiation
definition]{explicit-instantiation \emph{definition}}, though rarely used,
  was \emcppsgloss[explicit instantiation
directive]{explicit-instantiation \emph{directive}}. The term
  \emcppsgloss[explicit instantiation
directive]{explicit-instantiation directive} was clarified in
  C++11 and can now also refer to syntax that is used to create a
  \emph{declaration} --- i.e., \emcppsgloss[explicit instantiation
...
\noindent This \emcppsgloss[explicit instantiation
directive]{explicit-instantiation directive} compels the compiler to
...
C++11 introduced the \emcppsgloss[explicit instantiation
declaration]{explicit-instan\-ti\-ation \emph{declaration}},
a complement to the \emcppsgloss[explicit instantiation
definition]{explicit-\linebreak[3]instan\-ti\-ation \emph{definition}}. The
newly provided syntax allows us to place
\lstinline!extern!~\lstinline!template! in front of the declaration of an
...
inlined --- in precisely the same way they would have been
...
Finally, note that the use of \emcppsgloss[explicit instantiation
directive]{explicit-instantiation
\emph{directives}} has absolutely no effect on the logical meaning of a
well-formed program; in particular, when applied to specializations of
function templates, they have no effect on overload
resolution:
...
\emcppsgloss[explicit instantiation
declaration]{explicit-instantiation declarations}. Only \emph{after}
...
So far, we have seen the use of \emcppsgloss[explicit instantiation
declaration]{explicit-instantiation declarations} and \emcppsgloss[explicit instantiation
definition]{explicit-instan\-tiation definitions} applied to only a
...
specific case, we own the component that requires
...
template class Vector<int>;
    // Generate object code for all non(ù{\codeincomments{template}}ù) member functions and definitions
    // of (ù{\codeincomments{static}}ù) data members of template (ù{\codeincomments{my::Vector}}ù) having (ù{\codeincomments{int}}ù) elements.
...
template std::size_t Vector<double>::length() const;  // BAD IDEA
    ...
    // parameter type (e.g., (ù{\codeincomments{double}}ù)).
...
separate \emcppsgloss[explicit instantiation
definition]{explicit-instantiation definition}. These
...
the built-in type \lstinline!int!. Explicit instantiation of individual member functions, such as \lstinline!length()! in the example above, is, however, only rarely useful; see \intraref{annoyances-externtemplate}{all-members-of-an-explicitly-defined-template-class-must-be-valid}.

Having installed the necessary \emcppsgloss[explicit instantiation
definition]{explicit-instantiation
definitions} in the component's\linebreak[4]%%%%%
 \lstinline!my_vector.cpp! file, we
...
corresponding \emcppsgloss[explicit instantiation
declaration]{explicit-instantiation declarations} to
...
in the example above are \emcppsgloss[explicit instantiation
declaration]{explicit-instantiation
...
\lstinline!extern!~\lstinline!template! declarations must appear in \lstinline!my_header.h! \emph{after} the declaration of the
...
To illustrate the effect of \emcppsgloss[explicit instantiation
declaration]{explicit-instantiation
declarations} and \emcppsgloss[explicit instantiation
definition]{explicit-instantiation
definitions} on the contents of object and executable files,
we'll use a simple \lstinline!lib_interval! library \emcppsgloss[components]{component}
...
\noindent This library component above defines, in the namespace \lstinline!lib!, an implementation of (1) a class template,
\lstinline!Interval!, and (2) a function template, \lstinline!intersect!.
...
multiple object files. By contrast, this file also defines the \emph{strong} symbol
\lstinline!main!, marked here by a \lstinline!T!. Linking \lstinline!app.o! with any
other object file containing such a symbol would cause the linker to report a
...
\noindent As the textual output above confirms, the final program contains exactly one copy of each weak symbol. In this tiny illustrative example, these weak symbols have been defined in only a single object file, thus not requiring the linker to select one definition out of many.

%As the textual output above confirms, each of the needed \emph{weak}
...
often leading to duplicate code \emcppsgloss[section]{sections} for implicitly
...
respective and hopefully identical weak-symbol \emcppsgloss[section]{sections} to
include in the final executable.

Imagine now that our program includes a large number of object files, many of which make use of our \lstinline!lib_interval! component,
...
templates related to just \lstinline!double! type with the intent of later putting them all in one place, i.e., the currently
...
\emcppsgloss[explicit instantiation
declaration]{explicit-instantiation declarations} --- one for the
\emph{class} template, \lstinline!Interval<double>!, and one for the
\emph{function} template,
...
relating to the class and function templates
...
instantiated definitions is in no way affected. \emcppsgloss[undefined symbols]{Undefined
...
place to sequester those definitions would be within the object
...
need to use \emcppsgloss[explicit instantiation
definition]{explicit-instantiation
...
\noindent The application object file, \lstinline!app.o!, naturally remained
...
instructed the linker to remove all unused code \emcppsgloss[section]{sections} from the
...
unused \emcppsgloss[section]{sections}, where each section contains, for example, its own
...
The \lstinline!extern!~\lstinline!template! feature is provided to
enable software architects to reduce code bloat in individual
object files for common instantiations of class,
...
times. \emcppsgloss[explicit instantiation
declaration]{Explicit-instantiation declarations} do \emph{not}
...
#include <game_fixedarray.h>  // (ù{\codeincomments{game::FixedArray}}ù)
#include <game_unit.h>        // (ù{\codeincomments{game::Unit}}ù)
...
\noindent Whether optimization involving \emcppsgloss[explicit instantiation
directive]{explicit-instantiation
...
Even before the introduction of \emcppsgloss[explicit instantiation
declaration]{explicit-instantiation
\emph{declarations}}, strategic use of \emcppsgloss[explicit instantiation
definition]{explicit-instantiation
\emph{definitions}} made it possible to \emcppsgloss{insulate} the
...
\noindent Without the two \emcppsgloss[explicit instantiation
definition]{explicit-instantiation definitions} in
...
\emcppsgloss[explicit instantiation
definition]{explicit-instantiation definitions} in the header file
...
result from client use; hence, the two \emcppsgloss[explicit instantiation
declaration]{explicit-instantiation
...
%% potentially add to the next edition: from Marshall Clow:
%Also, in the “Pitfalls” you failed to mention the one that I find the worst:
%
%If you explicitly instantiate template definitions in a shared library, you can’t ever change that w/o rebuilding every client of the library.
%This is especially annoying if you discover that one function is a performance bottleneck, and you would like to inline it.
%But if you mark it as “inline”, it won’t get placed into the dylib, and then programs that use the dylib and depend on that symbol will fail to load.
...
\item{The specific definition of each \lstinline!C<T>! relevant to an explicit specialization being optimized — including general, partial-specialization, and full-specialization definitions — must appear in the header file prior to its corresponding \emcppsgloss[explicit instantiation
declaration]{explicit-instantiation declaration}.}
\item{Each \emcppsgloss[explicit instantiation
declaration]{explicit-instantiation declaration} for each specialization of each separate top-level — i.e., class, function, or variable — template must appear in the component’s \lstinline!.h! file \emph{after} the corresponding general template declaration and the relevant general, partial-specialization, or full-specialization definition, but, in practice, always after \emph{all} such definitions, not just the relevant one.}
\item{Each template specialization having an \emcppsgloss[explicit instantiation
declaration]{explicit-instantiation declaration} in the header file must have a corresponding \emcppsgloss[explicit instantiation
definition]{explicit-instantiation definition} in the component’s implementation file, \lstinline!c.cpp!.}
\end{enumerate}
...
Finally, unless we provide a matching \emcppsgloss[explicit instantiation
definition]{explicit-instantiation
definition} in the \lstinline!c.cpp! file for each and every
corresponding \emcppsgloss[explicit instantiation
declaration]{explicit-instantiation declaration} in the
...
Additionally, any \emcppsgloss[explicit instantiation
definition]{explicit-instantiation definition} in
the \lstinline!c.cpp! file that is not accompanied by a corresponding
\emcppsgloss[explicit instantiation
declaration]{explicit-instantiation declaration} in the \lstinline!c.h!
...
because they still reside in their own \emcppsgloss[section]{sections} and are marked
...
template void f<int>(int v);                   // OK, matched in (ù{\codeincomments{c.h}}ù)
template void f<double>(double v);             // Bug, unmatched in (ù{\codeincomments{c.h}}ù) file
\end{emcppslisting}
\vspace*{2ex}   %%%% extra space to highlight different snippets
\begin{emcppslisting}[emcppsbatch=e7]
// client.cpp:
...
fails to link, because no definition of \lstinline!f<char>! is guaranteed to
...
\lstinline!extern!~\lstinline!template! has \emph{absolutely no effect} on
...
within some designated object file, one must consider that not all programs necessarily need
every (or even any) such instantiation. Special consideration should be given to classes that
have many member functions but typically use only a few.

For such classes, it might be beneficial to explicitly instantiate individual member functions instead of the entire class template.  However, selecting \emph{which} member functions to explicitly instantiate and with \emph{which} template arguments they should be instantiated without carefully measuring the effect on the overall object size might result in not only overall pessimization, but also to an unnecessary maintenance burden. Finally, remember that one
...
%substitute.
%
%If one suspects that \emcppsgloss[explicit instantiation
%directive]{explicit-instantiation directives}
...
%explicitly tell the linker to strip unused \emcppsgloss[section]{sections} resulting,
...
%%% PRODUCTION: John may want to add a citation to the "note that" footnote below, if he can find the relevant material.
...
physical \emcppsgloss[component]{components} --- i.e., \lstinline!.h!/\lstinline!.cpp! pairs
--- neither of which depends physically on the other. That is, an
application using just one of these components could be
...
for a user-defined type is similar to that of specializing an
  arbitrary user-defined trait for a user-defined type. JOHN: ADD LAKOS20 CITATION HERE. %  <!-- TODO: LAH: Add this footnote citation during revisions when/if John finds what he is seeking. "; see [lakos20]{.citation}, section x.y.z, pp. xxx-yyy, especially footnote nn, p. zzz." <!-- JLakos: I looked for this, I know we have two paragraphs on it, but I just cannot find it WE NEED TO FIND IT DARN INT!  IT'S A GOOD FOOTNOTE. STILL TBD JSL 201205_1827-->
  }} There are two
obvious alternatives:
\begin{enumerate}
\item{\lstinline!vector!: In this case, \lstinline!vector.h! would hold  \lstinline!extern!~\lstinline!template!~\lstinline!class!~\lstinline!Vector<String>;! --- the \emcppsgloss[explicit instantiation
declaration]{explicit-instantiation \emph{declaration}} --- and \lstinline!vector.cpp! would hold\linebreak[4]%%%%%
\lstinline!template!~\lstinline!class!~\lstinline!Vector<String>;! --- the \emcppsgloss[explicit instantiation
definition]{explicit-instantiation \emph{definition}}. With this approach, we would create a physical dependency of the \lstinline!vector! component on \lstinline!string!. Any client program wanting to use a \lstinline!Vector! would also depend on \lstinline!string! regardless of whether it was needed.}
...
fortunately, in this case, there is no \emcppsgloss{one-definition rule (ODR)} violation if they don't.
...
In general, when using a class template, only those members that are
...
\emcppsgloss[explicit instantiation
definition]{explicit-instantiation definition}, however, \emph{all}
members of a class template are instantiated.
...
struct U
{
    U(int i) { /* construct from (ù{\codeincomments{i}}ù) */ }
    // ...
};
...
although providing an \emcppsgloss[explicit instantiation
declaration]{explicit-instantiation declaration}
for \lstinline!W<U>! is valid, a corresponding
\emcppsgloss[explicit instantiation
definition]{explicit-instantiation definition} for \lstinline!W<U>!
...
code bloat is to provide \emcppsgloss[explicit instantiation
directive]{explicit-instantiation
directives} for each valid member function of \lstinline!W<U>!, an approach
...
\begin{itemize}
\item{\seealsoref{variable-templates}{\seealsolocationb}Extension of the template syntax for defining a family of like-named variables or static data members that can be instantiated explicitly}
\end{itemize}
