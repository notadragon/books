...
A variable declared at function (a.k.a. local) scope has \romeogloss{automatic storage duration}, 
except when it is marked \lstinline!static!, in which case it has \romeogloss{static storage duration}.
Variables having \romeogloss{automatic storage duration} are allocated on the stack
each time the function is invoked, and initialized when that invocation's \romeogloss{flow of control}
passes through the \romeogloss{definition} of that object.
In contrast, variables with static \romeogloss{storage duration} (e.g., \lstinline!iLocal!)
...
\romeogloss{multithreading context} because it was subject to \romeogloss{data races}   
...
have led to a \romeogloss{data race} during the initialization of
\lstinline!localLogger!, which was defined as a local \lstinline!static!
object in \lstinline!getLogger!.   This \romeogloss{undefined behavior} might have resulted in
...
\noindent As of C++11, the example above has no \romeogloss{data races} provided that \linebreak%%%%%%
...
function-scope \lstinline!static! variable is \romeogloss{undefined behavior} and is
...
perhaps latent) \romeogloss{undefined behavior}.
...
to initialize \lstinline!dz! has \romeogloss{undefined behavior} because the control flow reached the
...
a simple  \romeogloss{Meyers singleton} for a user-defined type with a
\romeogloss{user-provied} default constructor:
