...
% 6 March 2021, to copyediting
% 19 March 2021, copyedits in and proofed
% 21 March 2021, in FPPs
...
\emcppsFeature{
    ...
    long={Thread-Safe Function-Scope {\SecCode static} Variables},
    toclong={Thread-Safe Function-Scope \lstinline!static! Variables},
...
\emph{double-checked lock pattern}; see \intraref{appendix-functionstatic}{appendix:-c++03-double-checked-lock-pattern}.
...
\lstinline!std::abort!, that explicitly do \emph{not} destroy
  \emcppsgloss{static storage duration} objects. This behavior is as if each static object is scheduled for destruction by using the C Standard Library function \lstinline!std::atexit! right after construction.
...
Here we have an example of the ``Singleton pattern''\footnote{\cite{gamma95}, Chapter~3, section~``Singleton,'' pp.~127--134} being used to create the shared \lstinline!Logger! instance
...
with our earlier \lstinline!Logger! example from \intrarefsimple{logger-example},
...
have led to a \emcppsgloss[data races]{data race} during the initialization of
...
likely to result in deadlock; see \intraref{potential-pitfalls-functionstatic}{dangerous-recursive-initialization}.
...
The guarantees surrounding access across \emcppsgloss[translation unit (TU)]{translation units} to runtime
...
#endif
\end{emcppslisting}
\vspace*{2ex} % for some extra space
...
file or namespace scope in separate \emcppsgloss[translation unit (TU)]{translation units} will be
initialized just because a function located within that \emcppsgloss[translation unit (TU)]{translation unit}
happens to be called.

An effective pattern for helping to ensure that a nonlocal object
\emph{is} initialized before it is used from a separate \emcppsgloss[translation unit (TU)]{translation unit}
...
initialize the logger; see \intraref{appendix-functionstatic}{appendix:-c++03-double-checked-lock-pattern}.
...
e.g., perhaps using the \emcppsgloss[strong typedef idiom]{strong-\lstinline!typedef!\linebreak[4]%%%%%
\newpage
 idiom} (see \featureref{\locationa}{inheriting-constructors}), corresponding to each singleton\linebreak[4]%%%%%
 use:
...
a point where the variable can be referenced, no analogous guarantees are made of nonlocal objects of \emcppsgloss{static storage duration}.
This makes any interdependency in the initialization of such objects, especially across
\emcppsgloss[translation unit (TU)]{translation units (TUs)}, an abundant source of insidious
errors.

Objects that undergo \emcppsgloss{constant initialization} have no such issue:
...
until their constructors run, which itself might lead to \emcppsgloss{undefined behavior} that is not necessarily conspicuous.
...
\noindent These two \emcppsgloss[translation unit (TU)]{translation units} will be initialized before \lstinline!main! is
...
\noindent In other words, even though no order exists in which the \emcppsgloss[translation unit (TU)]{translation units}
...
While on the surface it may seem as though local and nonlocal objects of \emcppsgloss{static storage duration}
...
\begin{emcppslisting}
int fz(int i)  // The behavior of the first call is undefined unless (ù{\codeincomments{i}}ù) is 0.
{
...
Within any given \emcppsgloss[translation unit (TU)]{translation unit}, the relative order of initialization
...
object outside of the current \emcppsgloss[translation unit (TU)]{translation unit}, before \lstinline!main! is
...
ensure that no such uninitialized use occurs, even across \emcppsgloss[translation unit (TU)]{translation units}
...
reside within the same \emcppsgloss[translation unit (TU)]{translation unit} --- is not clearly known at compile time,
...
scope and resides in a separate \emcppsgloss[translation unit (TU)]{translation unit} or (2) any other
function-scope \lstinline!static! object (i.e., including one in the same
\emcppsgloss[translation unit (TU)]{translation unit}). For example, suppose we have implemented a low-level
...
\item{Otherwise, the program will have \emcppsgloss{undefined behavior} because the destructor of\linebreak[4]%%%%%
...
\emcppsgloss[user provided]{user-provided} default constructor:
...
instructions if the \emcppsgloss[user provided]{user-provided} constructor of \lstinline!S!
  is either removed or defaulted
   (see \featureref{\locationa}{defaulted-special-member-functions});
...
\subsection[Appendix]{Appendix}\label{appendix-functionstatic}
