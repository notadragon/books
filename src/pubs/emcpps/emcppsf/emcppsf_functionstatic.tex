...
% 8 Feb 2021 JMB, fixes to normalize include comments
...
the variable is declared at \emcppsgloss{function scope} (a.k.a.
\emcppsgloss{local scope}). An object (e.g., \lstinline!iLocal!) that is
declared \lstinline!static! within the body of a function (e.g.,
\lstinline!f!) will be initialized the first time the \emcppsgloss{flow of control} passes through the \emcppsgloss{definition} of that object:
...
\emph{past} the \emcppsgloss{definition} of a \lstinline!static! object until
...
%  copyediting. TBD. Lori: Not after copyediting; that's too late.
since only one thread of control is allowed to
grab the lock to enter the \emcppsgloss{critical section} that
...
\emcppsgloss{multithreading context} because it was subject to \emcppsgloss{data races} if the function was called concurrently from multiple threads.
...
\emcppsgloss{leaks} the memory for the previously constructed
\lstinline!MyString!. What's more, when the second destruction of the
string eventually occurs (after exiting \lstinline!main!),
\emcppsgloss{undefined behavior} will inevitably result, if it hasn't
already.

In practice, however, \emcppsgloss{undefined behavior} (prior to C++11) might
...
\lstinline!s! object would be \emcppsgloss{undefined behavior}, because its
lifetime has ended, even though its destructor did not run. Hence,
\emcppsgloss{undefined behavior} could manifest long before the second
destructor is run at the end of the program.

As of C++11, a conforming compiler is now required to ensure that
initialization of \emcppsgloss[function scope]{function-scope} \emcppsgloss[static duration]{static-duration}
...
guarantee is \emph{not} extended for other \emcppsgloss[static duration]{static-duration}
...
with a \emcppsgloss{callable object} \lstinline!functor!, a new thread
invoking \lstinline!functor! will be spawned. Note that
\lstinline!std::thread!'s destructor will \emph{not} \emcppsgloss{join} the
...
the logger) were not. That is to say, the implicit \emcppsgloss{critical section} that is guarded by the compiler includes evaluation of the
...
guaranteed to live past the end of \lstinline!main!. The \emcppsgloss[meyers singleton]{Meyers Singleton} pattern also gives us a chance to catch and respond to
...
now also possible. By creating an instance of the \emcppsgloss{RAII}-type
...
across translation units an abundant source of insidious errors. Objects that undergo \emcppsgloss{constant initialization} have no issue:
...
// b.cpp: (same idea as (ù{\codeincomments{a.cpp}}ù))
\end{emcppslisting}
...
\emcppsgloss[function scope]{function-scope} \emcppsgloss[static duration]{\lstinline!static!-duration} variable might
...
\emcppsgloss[user provided]{user-provided} (inline) default constructor:
