...
Prior to C++11, only \emcppsgloss{trivial types} --- e.g.,
\emcppsgloss{fundamental types}, such as \lstinline!int! and \lstinline!double!,
enumerated or pointer types, or a C-style array or \lstinline!struct!
(a.k.a. a \emcppsgloss{POD}) --- were allowed to be members of a
\lstinline!union!. This limitation prevented any user-defined type having
a \emcppsgloss[non trivial special member function]{non-trivial special member function} from being a member of a
\lstinline!union!:
...
\lstinline!d_s! above, allowing any type other than a \emcppsgloss{reference type} to be a member of a \lstinline!union!.
...
\emcppsgloss[non trivial constructor]{non-trivial constructor}, such as \lstinline!struct!~\lstinline!Nt!
below, must be constructed manually (e.g., via \emcppsgloss[placement new]{placement \lstinline!new!}) before it can be used:
...
\noindent As an added safety measure, any non-trivial \emcppsgloss{special member function} defined --- either implicitly or explicitly --- for any
\emcppsgloss{member} of a \lstinline!union! results in the compiler implicitly
deleting (see %Section~\ref{deleted-functions},
``\titleref{deleted-functions}" on page~\pageref{deleted-functions}) the corresponding \emcppsgloss{special member function} of the \lstinline!union! itself:
...
\noindent Notice that we have employed \emcppsgloss[placement new]{placement \lstinline!new!} syntax to
...
\emcppsgloss{undefined behavior} for the non-trivial \lstinline!Nt! type:
...
yet specifically addressed those \emcppsgloss[special member function]{special member functions}:
...
\subsubsection[Implementing a \emcppsgloss{sum type} as a discriminated {\tt union}]{Implementing a sum type as a discriminated {\SubsubsecCode union}}\label{implementing-a-sum-type-as-a-discriminating-(or-tagged)-union}

A \emcppsgloss{sum type} is an algebraic data type that provides a choice
...
\emcppsgloss{sum type} \lstinline!ParseResult! (see below), containing either an
...
of the \lstinline!ParseResult! \emcppsgloss{sum type}, we can achieve the
desired semantics:
...
\noindent If a \emcppsgloss{sum type} comprised more than two types, the discriminator would be an appropriately-sized integral or enumerated type instead of a Boolean.
...
\end{emcppslisting}
In practice, \lstinline!ParseResult! would typically use a more general \emcppsgloss{sum type}{\cprotect\footnote{\lstinline!std::variant!, introduced
in C++17, is the standard construct used to represent a \emcppsgloss{sum type} as a \emph{discriminated union}. Prior to C++17,
\lstinline!boost::variant! was the most widely used \emph{tagged} union
  implementation of a \emcppsgloss{sum type}.}} abstraction to support arbitrary value types and provide proper exception safety.
...
\subsubsection[Inadvertent misuse can lead to latent \emcppsgloss{undefined behavior} at runtime]{Inadvertent misuse can lead to latent \emcppsgloss{undefined behavior} at runtime}\label{inadvertent-misuse-can-lead-to-latent-undefined-behavior-at-runtime}
...
\emph{member initializer list} or \emcppsgloss[placement new]{placement \lstinline!new!}) or
accessing a different object than the one that was actually initialized
can result in tacit \emcppsgloss{undefined behavior}. Although forgetting to
destroy an object does not necessarily result in \emcppsgloss{undefined behavior}, failing to do so for any object that manages a resource such
