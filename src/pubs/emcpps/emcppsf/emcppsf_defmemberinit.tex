...
S4 s4{"goodbye", 0};          // OK, (ù{\codeincomments{s4.d\_c}}ù) initialized to (ù{\codeincomments{'g'}}ù).
\end{emcppslisting}
...
public:
    ...
    StateMachine(const StateProgram &program)  // Run the specified (ù{\codeincomments{program}}ù).
    : d_state(e_INIT),
    , d_program(program)
    { }

    StateMachine(StateProgram &&program)  // Run the specified (ù{\codeincomments{program}}ù).
    : d_state(e_INIT),
