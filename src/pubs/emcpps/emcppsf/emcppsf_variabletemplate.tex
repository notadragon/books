...
By beginning a variable declaration with the familiar \emcppsgloss[template head]{template-head} syntax --- e.g., \lstinline!template! \lstinline!<typename! \lstinline!T>! --- we can create a \emph{variable template}, which defines a family of variables
...
\noindent Variable templates, like \emcppsgloss[c style functions]{C-style functions}, may be declared at
...
parameters (including a \emcppsgloss{parameter pack}):
...
desired goal, does not consume memory in the \emcppsgloss{static data space}, and would also be applicable to non-integral constants.
