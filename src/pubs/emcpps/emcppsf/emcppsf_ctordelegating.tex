...
A \romeogloss{delegating constructor} is a constructor of a
\romeogloss{user-defined type (UDT)} --- i.e., \lstinline!class!, \lstinline!struct!, or
...
the name of the type as the only element in the \romeogloss{member
initializer list}:
...
the object being initialized is considered only \romeogloss{partially
constructed} (i.e., the object is not yet known to be in a valid state)
and hence its destructor will \emph{not} be
invoked{\cprotect\footnote{The destructor of a \romeogloss{partially
...
constructor, the object being initialized is considered \romeogloss{fully
...
factored out into a separate \romeogloss{value-semantic} class, say,
...
program is \romeogloss{ill-formed, no diagnostic required}. While some
...
necessarily exhibit \romeogloss{undefined behavior}. Clang 10.x, on the
...
suboptimal factoring --- e.g., in the case where the same \romeogloss{value}
is being presented in different forms to a variety of different
\romeogloss{mechanisms}. For example, consider the \lstinline!IPV4Host! class
...
separate \romeogloss{value semantic} type, e.g., \lstinline!IPV4Address!, to
