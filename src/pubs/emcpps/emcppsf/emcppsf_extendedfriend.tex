...
\emcppsgloss[user defined type]{user-defined type} (UDT) grants a designated type (or
...
\emcppsgloss[in process]{in-process}{\cprotect\footnote{When used to qualify a VST, the
term \emcppsgloss[in process]{in-process}, also called \emph{in-core}, refers to a type
...
value that is meaningful outside of the current process; see
  \cite{lakos2a}, section~4.2.}} \emcppsgloss[value semantic type]{value-semantic type (VST)}
...
\emcppsgloss{component} as that of class \lstinline!Window!, no
\emcppsgloss[long distance friendship]{long-distance friendship} results. The consequence of such a
...
Note that this use case involves \emcppsgloss[long distance friendship]{long-distance friendship}
inducing an implicit cyclic dependency between the \emcppsgloss{component}
...
\emcppsgloss[long distance friendship]{long-distance friendship} by widening the API for the
\lstinline!Window! class, the natural consequence of which would be to
invite public client abuse vis-a-vis \emcppsgloss[hyrum's law]{Hyrum's law}.
...
declarations can be helpful when implementing the \emcppsgloss[curiously recurring template pattern]{curiously recurring template pattern (CRTP)}. For use-case examples and more
...
\emcppsgloss[long distance friendship]{long-distance friendship}, was
...
\lstinline!CN-2!, each defined in its own physical \emcppsgloss{component},
...
\emcppsgloss{physical design}, which generally precludes \emcppsgloss[long distance friendship]{long-distance friendship}.}
\end{itemize}
...
mid-90s, which has since become known as the \emcppsgloss[curiously recurring template pattern]{curiously recurring template pattern (CRTP)}. The pattern is \emph{curious} because it
...
templates in general, is that we can take advantage of \emcppsgloss{duck typing} to achieve useful functionality in somewhat unusual ways. For
