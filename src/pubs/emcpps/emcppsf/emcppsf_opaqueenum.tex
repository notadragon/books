...
information \emph{hiding}: \romeogloss{encapsulation} and \romeogloss{insulation}.
...
a.k.a. a \romeogloss{cookie} --- to a client to hold and then present back to
...
Event-driven programming,\footnote{See also \cite{gamma95}, Chapter 5, section ``Observer,''  pp.~293--???} historically implemented using \romeogloss{callback
...
\romeogloss{ill-formed, no diagnostic required (IFNDR)}; see \intraref{potential-pitfalls-opaqueenum}{redeclaring-an-externally-defined-enumeration-locally}. Unless a separate ``forwarding'' header file is
...
embedding, for whatever reason, a \romeogloss{local declaration} of \lstinline!Event! instead:
...
physical coupling between components. Unlike a \romeogloss{forward class
...
that is \romeogloss{IFNDR}. In the \lstinline!client.cpp! example shown above, if the opaque
...
linked, the resulting program will be \romeogloss{IFNDR}:
