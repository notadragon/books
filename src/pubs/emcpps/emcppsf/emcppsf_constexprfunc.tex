...
formalized this notion with the term \emcppsgloss{manifestly constant evaluated} to capture all places where the value of an expression must
...
least one of its values can participate in \emcppsgloss[constant expression]{constant expressions}:
...
objects of this type in a \emcppsgloss[constexpr context]{\lstinline!constexpr! context}{\cprotect\footnote{This initialization can be made possible
...
time then it is \emcppsgloss[ill formed, no diagnostic required]{ill formed, no diagnostic required (IFNDR)}:
...
\noindent As with all \lstinline!inline! functions, it is an \emcppsgloss[one definition rule]{one-definition rule (ODR)} violation if definitions in different translation units
within a program are not token-for-token the same. If definitions do
differ across translation units, the program is \emcppsgloss[ill formed, no diagnostic required]{ill formed, no diagnostic required (IFNDR)}:
...
instead said ``is evaluated to determine the value of a \emcppsgloss{constant expression}.''
...
of \lstinline!ff(3)! and \lstinline!gg(4)! are \emcppsgloss[constexpr context]{\lstinline!constexpr! contexts} and are ill formed because the body of \lstinline!gg! has not yet
...
value cannot be read as part of evaluating a \emcppsgloss{constant expression}. If the function pointer \emph{is} \lstinline!constexpr! but
...
\emcppsgloss[function try block]{function-try-block}:
...
proceeds is a matter of \emcppsgloss[quality of implementation]{quality of implementation (QoI)}.
...
The criteria for determining whether a given type is a \emcppsgloss{literal type} can be divided into six parts:
...
when it points to a type that itself is \emph{not} a \emcppsgloss{literal type}.}
...
cannot be invoked as part of the \emcppsgloss{controlling constant expression} for preprocessor directives.
...
\lstinline!constexpr! function must be initialized with a \emcppsgloss{constant expression} prior to the definition of the function.
...
\noindent The macro above can be used in contexts where both a \emcppsgloss{constant expression} is required and the input is known only during program
execution:
...
\emcppsgloss[posix epoch]{POSIX epoch}, midnight on January 1, 1970. An important tool in
