...
generation of a \emcppsgloss{special member function} or to limit the types of
...
When instantiating an object of user-defined type, \emcppsgloss{special member functions} that have not been declared explicitly are
...
%For certain kinds of types, the notion of \emcppsgloss{copy semantics}
%including \emcppsgloss{move semantics} is not meaningful, and
%hence permitting the generation of copy operations is contraindicated. The two special member functions controlling \emcppsgloss[move operations]{\emph{move} operations}, introduced in C++11, are sometimes implemented as effective optimizations of \emcppsgloss[copy operations]{\emph{copy} operations} and much less frequently with \emcppsgloss[copy operations]{\emph{copy} operations} explicitly deleted; see \featureref{\locationc}{Rvalue-References}.
...
suppress generation of both the \emcppsgloss{copy constructor} and \emcppsgloss{copy assignment operator}. Prior to C++11, there was no direct way to express
...
\lstinline!long! or \lstinline!unsigned! integer involves a \emcppsgloss{standard conversion} (not just an \emcppsgloss{integral promotion}), the same as
...
When it comes to deleting certain \emcppsgloss{special member functions} of a class (or
...
\emph{six} of  the \emcppsgloss{special member functions} available as of C++11; see
\featureref{\locationa}{defaulted-special-member-functions}.
...
Deleting certain \emcppsgloss{special member functions} --- i.e., \emph{default} constructor,
