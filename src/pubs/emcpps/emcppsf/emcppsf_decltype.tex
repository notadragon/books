...
The keyword \lstinline!decltype! enables the compile-time inspection of the \romeoglossf{declared type} of an \romeoglossf{entity} \emph{or} the type and
\romeoglossf{value category} of an expression. Note that the special construct \lstinline!decltype(auto)! has a separate meaning; see \featureref{\locationf}{decltypeauto}.
...
If the operand is an unparenthesized \romeoglossf{id-expression} or unparenthesized member access, \lstinline!decltype! yields the \emph{declared type}, meaning the type of the \emph{entity} indicated by the operand:
...
\end{emcppslisting}
Using \lstinline!decltype(a!~\lstinline!+!~\lstinline!b)! as a return type is significantly different from relying on automatic \romeoglossf{\lstinline!return!-type
...
used in conjunction with \romeoglossf{SFINAE} (``Substitution Failure Is Not An Error") to validate an expression involving a
template parameter.

For example, consider the task of writing a generic \lstinline!sortRange!
function template that, given a \romeoglossf{range}, either invokes the
...
invoking the latter with the \lstinline!range! and a \romeoglossf{disambiguator}
...
template instantiation is commonly known as \romeoglossf{expression SFINAE},
...
Here, \lstinline!std::declval!, a function that cannot be executed at runtime and is only appropriate for use in \romeoglossf{unevaluated contexts}, produces an expression of the specified type.  When mixed with \lstinline!decltype!, this lets us determine the result types for expressions without needing to construct (or even being able to construct) objects of the needed types.
