\emcppsFeature{
    toclong={The Digit Separator: {\TOCCode '}},
...
\noindent For more information, see \intrarefsimple{appendix:-silent-loss-of-precision-in-floating-point-literals}.
...
literals to group bits in octets (\romeogloss{bytes}) or quartets
(\romeogloss{nibbles}), as shown in Table~\ref{digitseparator-table2}. In addition, using a binary literal with digits grouped in triplets instead of an octal literal to represent UNIX file permissions might improve code readability --- e.g., \lstinline!0b111'101'101! instead of |lstinline!0755!.\begin{table}[h!]
\begin{center}
\begin{threeparttable}
...
\begin{itemize}
\item{\seealsoref{binary-literals}{\locationb}Safe C++14 feature representing a binary constant for which digit separators are commonly used to group bits in octets (\romeogloss{bytes}) or quartets (\romeogloss{nibbles})}
\end{itemize}
...
\item[a]{Note that the most significant bit of the \romeogloss{mantissa}
...
(the sign of the exponent is encoded using \romeogloss{excess-$\mathbf{n}$}
notation).}
} % end footnotesize
...
\lstinline!float! (described above), when applied to a double-precision
(64-bit) \lstinline!double! and a quad-precision (128-bit)
