...
% 20 March 2021, altered appendix title typesetting
...
Use of \lstinline!=!\,\lstinline!default! in a \emcppsgloss[special member function]{special member function}'s
...
themselves. Determining which of the \emcppsgloss[special member function]{special member functions}
will continue to be generated and which will be suppressed in the
presence of \emcppsgloss[user provided special member function]{user-provided special member functions}
...
Hinnant's now famous table (see page~\pageref{default-table1} of \intrarefdoublenopage{appendix-default}{implicit-generation-of-special-member-functions})
...
\lstinline!=!\,\lstinline!default! to restore the constructor as a \emcppsgloss{trivial
...
\emcppsgloss{deleted function} participates in overload resolution and, if
...
\noindent A defaulted declaration may appear with any \emcppsgloss{access specifier} ---
...
treat the first declaration as a \emcppsgloss[user provided special member
function]{user-provided special member
function} and thus will suppress the generation of other
\emcppsgloss[special member function]{special member functions} accordingly.
...
functions. As an example, consider a \emcppsgloss[value semantic]{value-semantic}
...
because those are, respectively, the \emcppsgloss[default initialized]{default-initialized} values
...
\noindent Now suppose that we get a request to add a \emcppsgloss{value constructor}
...
explicitly often degraded performance compared to a \emcppsgloss{trivial} default. Hence, such
...
\emcppsgloss{trivial} functions into equivalent
non-\emcppsgloss{trivial} ones:
...
It can be beneficial if a particular type is \emcppsgloss{trivial}.  The type is considered \emcppsgloss{trivial} if its default constructor is \emcppsgloss{trivial} and it is \emcppsgloss{trivially copyable} --- i.e., it has no non-trivial copy or move constructors, no non-trivial copy or move assignment operators, at least one of those nondeleted, and a trivial destructor.  As an example, consider a simple \emcppsgloss{trivial}  \lstinline!Metrics! type in the code snippet below containing certain collected metrics for our application:
...
implementations of individual methods offers distinct maintenance advantages.\linebreak\newpage%%%%%%
Specifying that a special member
...
Library classes often rely on whether the type on which they are operating is eligible for being copied with \lstinline!memcpy! for optimization purposes.  Such could be the case for implementing, \pagebreak%%%%%
say, \lstinline!vector!, which would make a single call to \lstinline!memcpy! when growing its buffer.
For the \lstinline!memcpy! or \lstinline!memmove! to be well-defined, however, the type of the object that is stored in the buffer must be \emcppsgloss{trivially copyable}.  One might assume that this trait means that, as long as the copy constructor of the type is trivial, this optimization will apply.  Defaulting the copy operations would then allow us to achieve this goal, while allowing the type to have a non-trivial destructor or move operation.  Such, however, is not the case.

The requirements for a type to be considered \emcppsgloss{trivially copyable} --- and thus eligible for use with \lstinline!memcpy! --- include triviality of all of its nondeleted copy and move operations as well as of its destructor.  Furthermore, library authors cannot perform fine-grained dispatch based on which operations on the type are in fact trivial.  Even if we detect that the type is trivially copy-constructible with the \lstinline!std::is_trivially_copy_constructible! trait and know that our code would use only copy constructors (and not copy assignment nor any move operations), we still would not be able to use \lstinline!memcpy! unless the more restrictive \lstinline!std::is_trivially_copyable! trait is also \lstinline!true!.
%%%%%%%%%%%%%%%%
...
\begin{itemize}
\item{\seealsoref{deleted-functions}{\seealsolocationa}describes a companion feature, \lstinline!=!\,\lstinline!delete!, that can be used to suppress access to implicitly generated \emcppsgloss[special member function]{special member functions}.}
\item{\seealsoref{compile-time-assertions-(static_assert)}{\seealsolocationa}describes a facility that can be used to verify at compile time that undesirable copy and move operations are declared to be accessible.}
\item{\seealsoref{Rvalue-References}{\seealsolocationc}provides the bases for \emcppsgloss{move operations}, namely, the move-constructor and move-assignment \emcppsgloss[special member function]{special member functions}, which too can be defaulted.}
\end{itemize}
...
operations not being declared. If more than one \emcppsgloss[special member function]{special member
