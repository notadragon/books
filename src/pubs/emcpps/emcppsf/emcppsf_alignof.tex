...
\emcppsgloss{complete type}, a \emcppsgloss{reference type}, or an \emcppsgloss{array type}. If \lstinline!T! is a \emcppsgloss{complete type}, the result is the
...
\lstinline!alignof(FPT)! is platform-dependent but is typically approximated well by the type's \emcppsgloss{natural alignment} --- i.e.,
...
The alignment of the base type always affects the derived type's alignment. However, this is observable only for an empty base if it is \emcppsgloss[over aligned]{over-aligned} (see \featureref{\locationc}{alignas}):
...
\noindent The same sort of issue occurs in conjunction with modern \emcppsgloss{type inference} features such as \lstinline!auto! (see \featureref{\locationc}{auto-feature}) and
