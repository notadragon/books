...
\noindent Additionally, the notion of \emcppsgloss[contextual convertibility to bool]{contextual convertibility to \lstinline!bool!} 
...
%  so the cumbersome \emcppsgloss[safe bool idiom]{safe-\lstinline!bool! idiom} was used instead,
...
%  used in lieu of the (now deprecated) \emcppsgloss[safe bool idiom]{safe-\lstinline!bool! idiom}; see
%  \cite{sharpe13}.}} 
...
%conveniently) via the \emcppsgloss[safe bool idiom]{safe-\lstinline!bool! idiom}.
...
\emcppsgloss{conversion operator} could easily allow accidental misuse to go
undetected:
...
was to return some obscure pointer\linebreak[4] %%%%%
 type (e.g., \emcppsgloss{pointer to member}) that could not possibly be useful in any context other\linebreak[4] %%%%%
...
\emcppsgloss[contextual conversion to bool]{contextual conversion to \lstinline!bool!}.{\cprotect\footnote{Note that two
consecutive \lstinline|!| operators can be used to synthesize a
\emcppsgloss[contextual conversion to bool]{contextual conversion to \lstinline!bool!} --- i.e., if \lstinline!X! is an
