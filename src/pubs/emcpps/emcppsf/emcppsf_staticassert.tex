...
\emcppsgloss[ill formed, no diagnostic required]{ill-formed, no diagnostic required (IFNDR)}, which was the case for
...
conjunction with standard \emcppsgloss{type traits} provided in
...
\emcppsgloss{IFNDR}. Attempting to prevent the
...
\emcppsgloss[ill formed]{ill-formed} and, in this simple case, will likely result in a
...
rights to refuse to compile this still technically \emcppsgloss[ill formed]{ill-formed}
program.
...
constraining overloads using \emcppsgloss[expression sfinae]{expression SFINAE}, see
